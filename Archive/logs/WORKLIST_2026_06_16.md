...
**Last updated:** 2026-06-16 EOD (§11v SH ≤2015 sweep + sentinel refactor + Aberdeen-eras findings draft). Day arc: Phase 0 calibration GREEN-LIGHT (100 obs / 0 HARD fails / 6.9 min) → schema extension 17→31 cols (post-SHA `83f97b38…`) → full sweep launched PID 59349 at 07:32 → completed 17:51 ET, 8,632 ok / 27 fail in 619.7 min → gates at scale 0 HARD fails + 2 healthy soft flags (G2c 3y / G8b) → parse-fail spot-check discovered 26 of 27 are Sonar refusals not parser bugs → Pete chose -99 sentinel for content_unrecoverable (preserves distinction from -1) → three-file sentinel batch committed `8c683bf2` → reclassification applied (16→-99, 11 stay as -1, post-SHA `ba99b96e…`) → test runner v1.1 shipped `0dc0c11d` then `05d70092` (min-n=30 threshold + n-desc sort) → headline finding pivoted from "era decline" to "methodology, not era" after Pete identified two confounds: (a) 2010s rows are Pete's post-Aberdeen work not Aberdeen output (cutoff retightened ≤2007), (b) author-presence bias from Pete's personal retention dominating the pre-Wayback era, not curation. Findings doc drafted under framing B (both T1 and T3 reported with full transparency on author-presence bias; T3 stands as cleaner finding; Kastner 2006-2007 SOA content acknowledged stands on its own). Draft `study_findings_prescience_decline_aberdeen_eras_v1.md` (228 lines, 19.4KB) shared to Pete for review; commit pending sign-off. Tier B Pass C still running PID 2163 at 83.5% (8,352 / ~10,000) as of 18:37.
...
- [x] **§11v Phase 0 CALIBRATION RUN — DONE 2026-06-16 AM.** GREEN-LIGHT: 100 obs / 0 HARD fails / 6.9 min. Schema extended 17→31 cols, post-SHA `83f97b38…`, backup at `_master_observations.csv.bak_pre_sh_extend_20260616T071946Z`.
- [x] **§11v SH ≤2015 full sweep — DONE 2026-06-16 17:51 ET.** 8,659 obs scored in 619.7 min within $500 ceiling. 8,632 valid scores, 27 sentinel fails, 0 HARD fails on gates v2 at scale, 2 healthy soft flags (G2c 3y / G8b). 3y mean 2.867, 5y mean 2.953, score 4+5 share 43.4% (3y), divergence 24.4%.
- [x] **§11v Sentinel scheme + refusal reclassification — DONE 2026-06-16 PM.** Locked: -99 content_unrecoverable, -1 parse_fail:malformed, -2 pending. Post-hoc reclass: 16→-99, 11 stay -1. Refusal manifest at `Perplexity_Only/sh_sweep_le_2015_refusal_manifest_v1.csv` for archive-hygiene work. Commit `8c683bf2`. Post-reclass SHA `ba99b96e…`.
- [x] **§11v Quality decline test runner — DONE 2026-06-16 PM.** `run_quality_decline_tests_v1.py` v1.1 (commit `05d70092`). T1 primary cohort ≤2007 Kastner-era Aberdeen (n=8,381) + T3 methodology-code with min-n=30 threshold (24 above + 265 below collapsed). T2 per-author analysis deliberately deferred.
- [ ] **§11v Aberdeen-eras findings doc — COMMIT PENDING.** Draft at `/home/user/workspace/study_findings_prescience_decline_aberdeen_eras_v1.md` (228 lines, 19.4KB). Target on commit: `Archive/decisions/study_findings_prescience_decline_aberdeen_eras_v1.md`. Framing B (full author-presence bias transparency throughout T1, T3 cleaner finding). Headline: methodology, not era. Kastner 2006-2007 SOA content acknowledged. Awaiting Pete sign-off.
- [ ] **§11v Follow-on (deferred):** (a) Driver v9 bake -99 sentinel natively; (b) Methodology code normalization pass (492 codes → cleaner taxonomy, collapse `ia`/`industry-analysis`, `dr`/`document-review`, `primary-doc`/`primary-source`, `expert-opinion`/`expert-quote`/`expert-analysis`); (c) 16-obs archive-hygiene pass using refusal manifest; (d) Decade × methodology cross-tab; (e) Targeted 1990s archive recovery campaign for colleague archives; (f) Stratified within-collection-source T1 if recovery-method metadata can be added.
- [ ] **§11v T2 / Kastner-accuracy study (DEFERRED future study).** Per-author prescience ranking is its own study with its own framing, addresses author-presence bias on per-author terms. Separate future deliverable.
...
### 2026-06-16 AM (§11v Phase 0 calibration kickoff)
