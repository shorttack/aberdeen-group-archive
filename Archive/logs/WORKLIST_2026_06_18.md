...
**Last updated:** 2026-06-18 PM-2 (v1.7.0 ship-gate cleanup complete in sandbox — all four must-fix findings F2/F3/F6/F7 from §11v audit closed via three commits: `7935aec7` (F2 — promote_pass_c_to_master_v1.py byte-aligned to Mac §11v patch), `5f945dd9` (F3+F6+F7 batch — add_row_class_to_prescience_scores_v1.py 251 LOC + retag_cloud_parse_fails_v1.py 154 LOC + F7_preseed_skip_sh_treatment_decision_v1.md 89 LOC), `a6c7a007` (docs batch — PRESCIENCE_ARCHITECTURE rev2 + MASTERS_NOTES v3 + RELEASE_NOTES_v1_7_0). Pete's locked decisions: F3=A (`row_class` 12th column on _master_prescience_scores.csv, enum {scored,parse_fail,prefilter_skip,preseed_skip,no_anchor,pending}, expected 8119/64/4/253/0/0=8440), F6=A (retag 12 cloud parse-fails to `pass_c_cloud_parse_fail`, parse_ok unchanged, runs AFTER F3), F7=A (driver v8 scores preseed-skip normally in SH; preseed=long-horizon-only; parallel SH verdicts; Rule A SH uses `source_pass IN (pass_c_sh_3y,pass_c_sh_5y) AND prescience_score>=0`). **Next session:** Mac-side cutover dry-runs (F3 → F6 → Phase 1+2 → shape audit → surgical Phase 5), patch doc numbers if drift, then v1.7.0 tag + Release. Corpus shape unchanged in v1.7.0 (1453/23926/3276/4361/865-high): this is a schema-and-discipline release, not a corpus release.)
