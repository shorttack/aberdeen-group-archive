...
Why this exists: the A/B run that produced the top-6 lists was recorded once.
Rather than re-run the harness just to regenerate the labeling surface for the 12
queries we already have data for, we reconstruct the template here from the recorded
slugs. The 8 NEW probes (probes_v1.txt rows 13-20) have no recorded slugs yet — run
`embed_ab_harness_v2.py --emit-gold-template` to append their rows, then label the
whole file. This is a one-off convenience generator; new candidates should use the
harness's `--emit-gold-template` path directly.
...
OUT = Path("embed_gold_template_prefilled_v1.csv")
...
def main():
    ...
    print(f"  --emit-gold-template and append, OR label these 12 first.")
