...
**Lane:** embedding models only (bge-m3 → candidate). This is deliberately SEPARATE
from the `local-model-upgrade-gates` skill, which governs the *generative* LLM lane
and explicitly excludes embeddings ("bge-m3 lives in a different lane; use a separate
evaluation"). This document IS that separate evaluation.

**Core bias (identical to the LLM lane): status quo wins ties.** Every gate is
biased toward "keep the incumbent embedder". A candidate must earn its way through
all gates in order. Failing any one stops the process.

Why the bias: the cost asymmetry is real and worse for embeddings than for the LLM.
...
- Cost of promoting an embedding upgrade we shouldn't have: a full re-embed of the
  entire corpus (~10,900 pages, ~3 h wall-clock on the M4 Pro), plus every downstream
...
## What makes the embedding lane different from the LLM lane
...
2. **There is no "thinking mode" to worry about**, but there IS a
   ...
   Both are recorded in OLLAMA_GOTCHAS.md — read it (Gate 0).
...
1. Read `Perplexity_Only/OLLAMA_GOTCHAS.md` end-to-end (append-only landmine ledger).
   Hold any embedding-relevant entries in working context — especially the
   `/v1/embeddings` + `dimensions` + L2-normalize entry.
...
`dimensions:1024` → 1024. MRL truncation works on Pete's Ollama build. PASS.
...
**Recorded for this evaluation (Pete's MTEB table, 2026-06-30):**
...
Use Phase 5 v4 (`05_compute_embeddings_v4.py`) with the candidate model and
`--dimensions 1024 --out data/embeddings_qwen1024.parquet`. The schema (6-col:
...
3. **Pete labels** the `relevant` column 1/0 by hand.
...
## Anti-patterns (don't repeat the LLM lane's §11q)
