...
description: "Operating Pete Kastner's Aberdeen archive pipeline: modifying master CSVs, backfills, rebuilding the DuckDB + parquet layer, refreshing wiki markdown + embeddings, Pass C prescience scoring. Use for adding a column to a master, backfilling, correcting rows, rebuilding wiki/DuckDB, regenerating parquets/pages, refreshing scaffolding, re-embedding, fixing shape numbers, running Pass C, rolling up verdicts. Codifies the SH-aware Phase 1-6 chain via pipeline_canonical_v2.sh, shape-audit query, stale-embeddings gotcha, Pass C. v1.9 (2026-07-08 PM): Pass C v5->v7 (v5 retired to _legacy; v7 sweeps all unscored obs via master, no prepared/ whitelist, single output file, `pass_c_sonar_v1`/`v7` provenance). v1.8 (2026-07-08 AM): masters at repo root `aberdeen-group-archive/`; canonical versions 01_v3/02_v5/03_v3/04_v6/05_v3/06_v2 + Phase 0 gate; SH views; baseline 1504/24842/3288/4368/876-high; Gotcha 13."
metadata:
  author: pete-kastner
  version: '1.9'
---
...
## Pass C: v7 architecture + diagnosis tree (v1.9)

Pass C is cloud-API prescience scoring via Perplexity Sonar. **v1.9 (2026-07-08)** collapses the previous three-file architecture: v7 sweeps unscored observations directly against `_master_prescience_scores.csv` and writes results to a user-specified `--output` file, which is then merged into the master via `promote_pass_c_to_master_v1.py`. The old File 1 (`prescience_scores_pass_c_cloud_v1.csv`) is retired; v5 lives in `scripts/_legacy/` as of the 2026-07-08 legacy-moves batch.

### Canonical v7 workflow

| Artifact | Path | Notes |
|---|---|---|
| Runner (canonical) | `~/Desktop/Archive/aberdeen-group-archive/scripts/run_prescience_pass_c_v7.py` | **v7 (2026-07-01)**. Reads master, sweeps unscored obs, respects prescience=`not-applicable` studies. `--input-manifest <csv>` for calibration or batched subsets. `--output <csv>` separates per-batch output. `--limit N` for dry cost probes. |
| Score master | `~/Desktop/Archive/aberdeen-group-archive/_master_prescience_scores.csv` | 11 cols. Canonical scored-observations table. Both v7's resume logic and the promote script read this directly. |
| Promote | `~/Desktop/Archive/aberdeen-group-archive/scripts/promote_pass_c_to_master_v1.py` | Merges a per-batch `--output` CSV into the score master. Append-only, dedupes on `obs_id`. |
| Studies master (verdicts) | `~/Desktop/Archive/aberdeen-group-archive/_master_studies.csv` | Canonical for the `prescience` enum + rationale. Edit directly; commits ship at EOD. |
| Master obs (v7 reads) | `~/Desktop/Archive/aberdeen-group-archive/_master_observations.csv` | Full observation universe. v7 sweeps unscored rows from here. |
| Logs | `~/Desktop/Archive/logs/pass_c_sonar_v1_*.{md,jsonl}` | Per-run report, failures, prefilter audit. |
| API key | `~/.config/adoptex/perplexity.env` | sonar-reasoning-pro |

**v7 field values (differ from v5!):**
- `source_pass` = `pass_c_sonar_v1` for API-scored rows; `pass_c_prefilter_v1` for rule-based skips (was `pass_c_cloud` in v5)
- `scorer_version` = `v7` (was `cloud_v1` in v5)

When promoting v7 batches to the master, use the v7 defaults (they're the runner's actual output). When querying legacy File 2 rows to distinguish v5 from v7 provenance:
```sql
SELECT source_pass, scorer_version, COUNT(*) FROM v_prescience_scores GROUP BY 1, 2 ORDER BY 3 DESC;
```

### Path A — scorer-is-judge, math-driven (v7)

1. **Score** (calibration or full sweep):
   ```bash
   # Custom subset (calibration sample, Tier A batch, etc.)
   python3 scripts/run_prescience_pass_c_v7.py \
     --input-manifest /path/to/subset.csv \
     --output /Users/scott/Desktop/Archive/pass_c_v7_batch_<label>.csv \
     --limit 10  # dry cost probe first!

   # Full unscored sweep against master
   python3 scripts/run_prescience_pass_c_v7.py \
     --output /Users/scott/Desktop/Archive/pass_c_v7_full_<utc-stamp>.csv \
     --limit 5000
   ```
2. **Promote** the batch into the score master:
   ```bash
   python3 scripts/promote_pass_c_to_master_v1.py \
     --input /Users/scott/Desktop/Archive/pass_c_v7_batch_<label>.csv \
     --commit --scorer-version v7 --source-pass pass_c_sonar_v1
   ```
   (Adjust `--scorer-version` / `--source-pass` to match what the batch actually contains. If a batch mixed prefilter and API rows, promote in two invocations with different flags, or trust the CSV's per-row values if the promote script supports pass-through.)
3. **Compute verdict** (Rule A, below) over master rows for the study_id.
4. **Write verdict** into `_master_studies.csv` (repo root) `prescience` + `prescience_rationale` columns.
5. **Commit at EOD.** No separate sync step.

### Path B — player rebuttal overrides scorer

1. Steps 1+2 above still record raw scores in the master (for transparency).
2. Author rebuttal note at `kastner-author/notes/<study_id>_prescience_rationale_<date>.md`.
3. Author verdict + rationale directly in `_master_studies.csv` `prescience` and `prescience_rationale` columns.
4. Commit at EOD.
5. Scorer-is-judge satisfied: the player's argument is recorded alongside the verdict.

### v7 quirks

1. **Docstring still references 492 prepared studies** — that's v5 archaeology in the version-history block; v7 does not use `prepared/` at all.
2. **`--input-manifest` CSV must have `obs_id` and `study_id` columns.** v7 joins against the observations master by `obs_id`.
3. **Prefilter is still rule-based.** Prefiltered obs land in the output with empty `prescience_score`, `source_pass='pass_c_prefilter_v1'`. They count toward `n_total`, not `n_used`.
4. **Resume reads the canonical master** — v7 will skip any obs already present in `_master_prescience_scores.csv` (regardless of prior `source_pass`/`scorer_version`). To rescore, delete the row from the master first.
5. **`--limit` is a hard ceiling, not a rate.** Set it low on first run to bound cost.

### Retired v5 (do not run)

`scripts/_legacy/run_prescience_pass_c_v5.py` is preserved for forensic value only. Key differences vs v7 (in case you're reading old logs or the 2026-06-13 §11v decisions log):

- **v5 scope**: locked to 492 studies in `~/Desktop/Archive/prepared/<study_id>/` (a directory whitelist).
- **v5 output**: appended to `~/Desktop/Archive/prescience_scores_pass_c_cloud_v1.csv` ("File 1"). v5 read+wrote that same file for resume.
- **v5 field values**: `source_pass='pass_c_cloud'`, `scorer_version='cloud_v1'`.
- **v5 quirks that no longer apply**: hardcoded `obs in 492 prepared studies` string; `load_already_scored()` reading only File 1; no `--study-ids` flag.
- The old "three-file architecture" (File 1 live output / File 2 master / File 3 repo snapshot) is a v5-era mental model. Post-v6+: one master + per-batch output files. "File 3" (a repo snapshot of File 1) is no longer refreshed at all — the CSV that used to sit at `aberdeen-group-archive/prescience_scores_pass_c_cloud_v1.csv` is a frozen forensic artifact from the v5 era.
...
**Step 1 — obs exist in `_master_observations.csv`?** (v1.9: `prepared/` directory check removed — v7 does not use it)
```bash
...
**Step 2 — already scored in the master?**
```bash
python3 -c "
import csv
...
hits = sum(1 for r in csv.DictReader(open('/Users/scott/Desktop/Archive/aberdeen-group-archive/_master_prescience_scores.csv')) if r['obs_id'] in target)
print(f'in master: {hits} / {len(target)}')
"
```
Or via DuckDB (faster, if the pipeline has been rebuilt):
```bash
duckdb -readonly ~/Repos/kastner-aberdeen-wiki/db/kastner.duckdb -c "
SELECT COUNT(*) AS scored, (SELECT COUNT(*) FROM v_observations WHERE study_id='<study_id>') AS total
FROM v_prescience_scores p JOIN v_observations o USING (obs_id)
WHERE o.study_id = '<study_id>';
"
```
If all present: skip v7; compute verdict (Step 3), then Path A or B above.

**Step 3 — compute verdict directly** (no API call needed): use Rule A above.

**Step 4 — diagnostic: `Pass C says 0 to process`?** v7 is doing the right thing — all observations for this study are already scored (or the study is `prescience='not-applicable'` and v7 correctly skipped it). Confirm via Step 2's master-query; if hits == total, there is nothing to score.

### Pass C gotchas

- **v5 is retired.** Run `run_prescience_pass_c_v7.py`. Do not run `run_prescience_pass_c_v5.py` from `scripts/_legacy/` — its output writes to a retired path and uses the old `pass_c_cloud` provenance labels, which will confuse queries and downstream promote steps.
- **`--limit` first, `--commit` never automatic.** v7's `--output` writes new rows but the *master* is only updated by `promote_pass_c_to_master_v1.py`. Always run a small-limit dry probe (`--limit 10`) before an unlimited sweep to confirm the cost and prompt behavior.
- **`[DEFERRED]` studies may already be scored.** Check the master (`_master_prescience_scores.csv`) for the study_id's obs_ids before running v7. The 2026-06-13 §11v session burned credits planning an API run for 5 studies that turned out fully scored (4 all-prefiltered = not-applicable; 1 with 26 scored obs = low).
- **Legacy score rows have different provenance labels.** Rows from the v5 era carry `source_pass='pass_c_cloud'` + `scorer_version='cloud_v1'`; v6/v7 rows carry `pass_c_sonar_v1`/`v7`. If you're computing a study-level verdict via Rule A, both count — the raw scores are equivalent. If you're auditing provenance or re-running a subset, filter on these fields.
- **Roll-up v3 (`scripts/v3_obsolete/roll_up_prescience_v3.py`) is doubly deprecated.** It read the old File 2 and only flipped `[DEFERRED]`. Replaced by the manual write + EOD commit workflow described in Path A above.
- **Masters live at repo root** (post-2026-06-24). Edit `_master_studies.csv` directly; no sync script needed. Pre-2026-06-24 sessions may reference `archive_masters/_master_studies.csv` as "canonical" — that guidance is stale.

The companion runbook lives at `~/Desktop/Archive/aberdeen-group-archive/Perplexity_Only/PASS_C_RUNBOOK.md`. **The runbook is stale for v7 as of 2026-07-08 skill v1.9 patch** — update on next Pass C run.
