...
**Last updated:** 2026-06-12 PM-2 (§11u-cont **Pass B EXTRACTS COMPLETE in sandbox**. All 17 transcripts extracted: 295 obs + 194 entities + 122 techs. Consolidated into 4 batch files (1 REPLACE for studies, 3 APPEND), all §16 GREEN. EOD batch ships batch CSVs + apply script + decisions log + WORKLIST to archive repo. Mac merges via `apply_passb_transcripts_v1.py`, then runs Phase 1+2 + §17 Pass A + §18 + §20 + Phases 3-6.)
**Current ship state:** archive `origin/main` at `5c4c6105` post-§11t cleanup (parent `22a89d34` = §11t reconcile). Tonight's commits NOT YET pushed by sandbox. Mac state at end of §11u-cont AM: 1452 studies / 23631 observations / 3207 entities / 4312 techs. After tonight's Mac merge of batch_*.csv: expected **1452 studies / 23926 observations / 3401 entities / 4434 techs** (studies REPLACE not add; observations/entities/techs APPEND). `v_studies_with_high_prescience` likely +1 (SARS CNBC has ≥1 'high' confidence prediction observation) — verify post-Phase 1+2. Wiki `origin/main` unchanged from v1.6 release. bge-m3:latest canonical embedding model. Methodology vocabulary locked: `internal-sales-training-archive` (Blue Monday only), `vendor-event-archive` (12 vendor talks), `broadcast-archive` (4 TV news). **Security:** git `user.name` was "Catalina" (one of Pete's passwords); leak found 2026-06-01 PM, rotated to `shorttack`; 976 historical commits across the two public repos retain dead-string "Catalina" in Author metadata.
