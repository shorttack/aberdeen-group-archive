...
**Last updated:** 2026-06-12 PM (§11u DECtp Pass B + §11u-cont 17-transcript Pass A BOTH SHIPPED on Mac; Phase 1+2 reran clean. Studies 1435→1452 (+17), observations 23605→23631 (+26 DECtp). Pass B for the 17 new transcripts deferred to next session. EOD batch in progress — sandbox ships ingest script + manifest + decisions log + WORKLIST; Mac commits 2 master CSVs + 2 backup-tree entries.)
**Current ship state:** archive `origin/main` at `5c4c6105` post-§11t cleanup (parent `22a89d34` = §11t reconcile). Today's commits NOT YET pushed (Mac will push masters+backups; sandbox will PUT scripts/manifest/log/WORKLIST). Wiki `origin/main` unchanged from v1.6 release. Phase 1+2 v4 baseline post-§11u+§11u-cont: **1452 studies / 23631 observations** / 3207 entities / 4312 technologies / 1452 with pub_year / 6 decades. `v_studies_with_high_prescience` = 124 (unchanged — obs-rollup view; Pass B not yet run for the 17 new studies). `v_high_holistic_prescience` = 491 (+2 from SARS broadcasts prescience='high' in manifest). bge-m3:latest canonical embedding model. Methodology vocabulary locked: `internal-sales-training-archive` (Blue Monday only), `vendor-event-archive` (12 vendor talks), `broadcast-archive` (4 TV news). **Security:** git `user.name` was "Catalina" (one of Pete's passwords); leak found 2026-06-01 PM, rotated to `shorttack`; 976 historical commits across the two public repos retain dead-string "Catalina" in Author metadata.
...
- [ ] **§11u-cont Pass B — observation extraction for 17 new transcript studies** (next session's primary task). Order: SARS CNBC (high prescience, broadcast) first, then walk vendor-event-archive transcripts. Per-transcript: read full markdown, extract evidence-anchored OBS blocks, append to `_master_observations.csv` via the established 17-col schema. Re-run Phase 1+2 after batches, confirm `v_studies_with_high_prescience` budges (will only happen once SARS gets ≥1 high obs). After all 17 done: full Phase 3-6 rebuild covering 18 new studies (DECtp + 17 transcripts).
