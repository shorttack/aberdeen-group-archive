...
- [ ] **v1.8.0 NEXT — `promote_quotations_to_master_v1.py`** (raised 2026-06-19 PM §11x). Promote 334 corpus verdicts to master. Decision pending: append to `_master_observations.csv` with `row_class='kastner_quotation'`, or stand up new `_master_quotations.csv` at repo root. **Pre-promotion quality gates:** (1) **byline/boilerplate filter** — top-20 high-conf non-prescient contains degenerate rows like rid 1186 (`'-- Peter S. Kastner'`), 1180 (blog tagline), 1200 (`'have your user manual'`), 1132 (attribution boilerplate). Add `low_signal_flag` when quote <10 words AND contains analyst name possessive OR 'Kastner Blog'/'blog at'. (2) Manual review of 12 tiebreaker flips (731/872/873 medium→high; 933 medium→low). (3) Sample-audit ~10 rationales per bucket. (4) Decide low-bucket promotion (recommend yes, with bucket label). Requires D3 preauthorization.
- [ ] **v1.8.0 BACKLOG — Phase B P1 parse-fail forensic** (raised 2026-06-19 PM §11x). P1 tiebreaker parse-failed at 5/47 = 10.6% (rows 1146, 1173, 171, 1176, 84) vs Phase A P2 at 1.8%. All recovered via P2 fallback, not blocking. Hypothesis: long article bodies trigger `<think>` blocks without closing tags or mid-stream truncation. Inspect `raw_response` field for these 5 JSONL records; if confirmed, add defensive `<think>` stripping for v2 of corpus scorer (also relevant to Pass C v6).
