...
**Last updated:** 2026-06-19 PM (§11x v1.8.0 Quotations Corpus → Wiki SHIPPED — per-quote chunking, 334 wiki pages, 10,774-row embeddings.parquet, kw ask verified on Oracle/IBM/Itanium queries. Mac-side EOD commit: single `git push` from `~/Repos/kastner-aberdeen-wiki/` covering 334 quote pages + slim index + parquets + duckdb + embeddings + scaffolding. Sandbox 9 commits to aberdeen-group-archive. **Cumulative v1.8.0 spend ~$34 / $500 ceiling.** **Pre-2026-06-19 AM-3 baseline preserved below:** 2026-06-19 AM-3 (v1.8.0 route_v2 GREEN, horizon backfill SHIPPED, Q1 admit-orphan triage RESOLVED. Route_v2 fixed Gotcha-9 corpus headline_norm drift — 220 corpus rows had stored field != recomputed norm; v2 recomputes from headline and produces 474 P1 / 292 tuples (commit `56cc4622`). Backfill_v2 plugged `3` (SH-3y default) into 244 P1-eligible blank `forecast_horizon_years` cells with sidecar audit (commit `c38ff792`). Q1 admit-orphan triage via `diag_admit_orphan_sources_v2.py` (commit `914bdd4c`): 2 A-class recoverable from PDF segments (rids 494 IBM Navy + 1005 Dell Oracle Linux SAP), 8 E-class cited inside other corpus articles' bodies (rids 199, 374, 426, 458, 780, 1020, 1045, 1181). All 10 scorable on quote-text alone; recovery deferred to backlog. Three Gotcha-9 strikes today — producer/consumer contract verification mandatory pre-commit. Substrate trinity confirmed intact: 179 corpus articles / 328 unclaimed segments / 734 quote-only rows. Scorable v1.8.0 tuples = 220 (above 170 highlight-reel target). **Next:** `score_quotations_calibration_v1.py` A/B harness. **2026-06-19 AM-2 baseline preserved below:** (v1.8.0 silent-loss apply SHIPPED + substrate-cap finding documented. After substrate ship at `39aabb46` left only 105/425/375/712 partition, ran silent-loss discovery: drafted `discover_unindexed_kastner_quotes_v2.py` with reject-column semantics — blank/whitespace = ADMIT, any non-space char = REJECT. Found 125 candidates; Pete reviewed in Numbers as v3, marked 4 reject + 121 admit; `apply_unindexed_quotes_v3.py --commit` appended 121 rows (1087 → **1208**). Re-ran union: article corpus 105 → **179**, unclaimed 425 → **328**, Pipeline 1 → **464**, Pipeline 2 → **744** ✓. Then attacked the residual 437 `pdf_format_mismatch` rows via `diagnose_pdf_format_mismatch_v4.py`: 410/437 are terminal (PDF doesn't contain the article — source-document gap, not code gap), 20 are substrate-salvageable, 7 are Pete-review. Pipeline 1 cap is ~470-480 absent additional source PDFs. Next step: ship 27-row review CSV using the same reject-column pattern. **2026-06-19 AM-1 baseline preserved below:** (v1.8.0 substrate SHIPPED at archive `39aabb46` — three artifacts in `kastner-author/quotations/`: `article_corpus_v1.json` 105 articles + `_pdf_segments_unclaimed_v1.json` 425 salvage segments + `quote_only_rows_v1.csv` 712 Pipeline 2 rows; partition GREEN 375+712=1087. Two HIGH-PRIORITY items added at top of Next up: `route_quotations_to_horizon_v1.py` + `score_quotations_calibration_v1.py`. Next focus: extract more quotes from the 425-segment salvage substrate to expand Pipeline 1 beyond 105 articles. Today's earlier scoping context: scope the quotations corpus (1,087 quotes from `kastner-author/quotations/kastner_quotes_clean.csv`) for v1.8.0 ingest. Pete's overnight insight: the long-file full-article context may score materially differently from the isolated quote — unit-of-prescience decision (quote vs paragraph vs full article) is the gating question, NOT umbrella-vs-per-article granularity as previously framed. Three architectural questions to answer before any code: (1) unit of prescience, (2) where the long file lives in the repo, (3) calibration A/B on the 230-row highlight reel to compare quote-alone vs article-context scoring. No WORKLIST commits during scoping; v1.7.0 EOD state preserved.) **2026-06-18 PM-3 baseline preserved below:** (**v1.7.0 SHIPPED.** Tag `v1.7.0` at archive `bd819f4e` pushed; GitHub Release published 2026-06-18T22:37:52Z at [releases/tag/v1.7.0](https://github.com/shorttack/aberdeen-group-archive/releases/tag/v1.7.0). Mac-side cutover sequence: F3 dry-run aborted with "column already exists" — column had been added on a prior Mac session but 8,645 new Pass C rows landed with NULL `row_class` afterward; F3b script (`scripts/backfill_row_class_v1.py`, 215 LOC, sandbox-simulated to match Mac exactly) drafted mid-cutover and committed at `730ac65f`. F3b Mac `--commit` applied 8,649 mutations (4 rename `prefilter`→`prefilter_skip` + 8,645 NULL backfills); backup `bak_backfill_row_class_20260618T222707Z`. Phase 1+2 rebuild GREEN: shape 1453/23926/3276/4361/865-high (unchanged), prescience master 17,085 rows × 12 cols, zero NULL `row_class`, distribution scored=16708 / parse_fail=116 / prefilter_skip=8 / preseed_skip=253 / no_anchor=0 / pending=0. Phase 5 skipped (Perplexity_Only/ docs not in wiki embedding index — zero matches in `wiki/`). F3b commit `bd819f4e` pushed past branch-protection bypass warnings (PR rule + commit signing). Full v1.7.0 commit lineage: `7935aec7` (F2), `5f945dd9` (F3+F6+F7 batch), `a6c7a007` (docs batch), `577892d4` (PM-2 EOD), `730ac65f` (F3b mid-cutover), `bd819f4e` (F3b cutover). Pre-commit `git status` surfaced two unrelated items: `_master_observations.csv` working-tree mutation (16→30 cols, 14 unknown columns added Jun 16 07:19, predates today, same 23,927 rows — DEFERRED per D3) and 5 terminal-typo junk files from tee redirect quoting errors — deleted before commit. v1.7.0 is a schema-and-discipline release: row_class enum locked, cloud parse-fail bucket disambiguated, preseed-skip SH semantics documented, salvage trust manifest published, Rule A SH rollup canonical.)
...
- [ ] **v1.8.0 BACKLOG — `clean_blog_artifacts_from_quotes_v1.py`** (raised 2026-06-19 PM §11x). 11 rows in `_master_quotations_prescience.csv` flagged with `blog_scrape_contamination_flag=true`: footer text / share-button text / `Posted by anonymous` artifacts mixed into otherwise-substantive prescient predictions. Pattern set: `posted by anonymous`, `email thisblogthis`, `share to twitter`, `share to facebook`, `share to pinterest`, `blogging at`, `oncomputerstips`, `blog at oncomputers`, `no comments:`. **Critical: contamination is about TEXT quality, NOT verdict quality.** Affected row_ids: 1180, 1183, 1186, 1187, 1188, 1190, 1193, 1194, 1199, 1200, 1208. Disposition: write `clean_blog_artifacts_from_quotes_v1.py` that strips known patterns from `quote` field + re-scores via the v1.8.0 corpus scorer (Phase A P2 → Phase B P1 tiebreak → resolver) + updates the 11 rows in `_master_quotations_prescience.csv` + sidecar audit. Defer until needed; per-quote wiki pages already note contamination inline.
...
### 2026-06-19 PM (§11x v1.8.0 Quotations Corpus → Wiki — per-quote chunking)

- **Sidecar master `_master_quotations_prescience.csv` SHIPPED** at archive `3e4c1b66`. 334 rows × 31 cols (27 corpus + 4 audit: `blog_scrape_contamination_flag`, `scorer_version`, `source_pass`, `promoted_at`). Lives at repo root per Gotcha 12 / kastner-archive-pipeline v1.7. Bucket distribution: high 184 (55.1%), medium 84 (25.1%), low 66 (19.8%). 11 rows flagged for blog-scrape contamination.
- **Promote script lineage:** `promote_quotations_to_master_v1.py` (`464f104b`, false-positive predicate flagging short legit quotes), `_v2.py` (`48483679`, blog-artifact patterns but misnamed `low_signal_flag`), `_v3.py` (`3c396807`, renamed to `blog_scrape_contamination_flag` for honest text-vs-verdict audit semantics). v3 is canonical.
- **Wiki page generator lineage:** `build_quotations_corpus_page_v1.py` (`43b9cbe9`, analyst-grouped — discovered 334/334 = single-author Peter S. Kastner, grouping meaningless), `_v2.py` (`4ad11526`, monolithic 377 KB flat-by-bucket — embedded as single chunk, retrieval failed), **`build_quotations_per_quote_v1.py` (`6918d6e0`, canonical — 334 per-quote pages + slim index)**.
- **Per-quote chunking architectural lesson:** bge-m3 has ~8192-token effective context. A 377 KB single embedding produced a centroid of "methodology / bucket counts" rather than any specific prediction. Per-quote pages (~1-5 KB each) give each prediction its own retrievable embedding. **Verified `kw ask` post-rebuild surfaces quote pages as top hits on Oracle 1992/1994/1997, IBM mainframes 1987-93, and Itanium 2003-08 queries.**
- **Phase 1+2+4+5+6 rebuild GREEN.** Phase 3 deliberately skipped (hand-authored pages, not master-derived). Phase 5 v2 run: 10,774 pages (10,440 + 334 quotes), 1219s / 20.3 min wall-clock via bge-m3 1024-dim on M4 Pro GPU. `data/embeddings.parquet` regenerated. Phase 6 refreshed `README.md` / `AGENTS.md` / `chat-starter.md` / `Makefile` / `.gitignore` / `scripts/verify.py` / `scripts/semantic_search.py`.
- **`kw ask --k N` flag confirmed.** Default k=6 too narrow when queries need both tech-page context AND specific quote evidence. `--k 15` produces clean mixed retrieval (Oracle 1997 query: 5 quote pages + 6 tech pages + 1 study + 1 entity + 2 longitudinals).
- **Shape audit (post-rebuild):** Phase 1 reported `_master_studies.csv: 1453 rows, 16 cols` (+1 vs §11u-cont baseline 1452 — to investigate). Phase 2 `v_studies_with_high_prescience` = 865 (far above 124-125 baseline — likely view definition change since 2026-06-13, flag for skill update). `v_high_holistic_prescience` = 498 (was 491 at §11u-cont; +7 consistent with intervening Pass B/C activity). Canonical archive shape (studies / obs / ent / tech) UNCHANGED at 1453/23926/3276/4361 this session — no canonical masters touched, only the new sidecar.
- **Pete pushback codified:** "ISN'T THERE AN ARCHITECTURALLY SIMPLER APPROACH?" (ALL CAPS) — when agent over-scoped wiki rebuild as full-Phase 1-through-6 integration. Right call. Logged in compaction summary as standing user-frustration signal: choose simplest viable path, escalate only if user asks.
- **Cumulative v1.8.0 spend: ~$34 of $500 ceiling.** No new API calls this session (all promote/page-gen/Phase 5).
- **Mac-side commit (Commit 1, this EOD):** single `git add . && git commit && git push` on `~/Repos/kastner-aberdeen-wiki/` covering 334 new `wiki/quotations/quote-*.md` pages + overwritten slim index + 12 refreshed `data/*.parquet` + `db/kastner.duckdb` + `data/embeddings.parquet` (~63 MB) + refreshed scaffolding + refreshed Phase 4 outputs.
- **Sandbox commits today:** 9 to `aberdeen-group-archive` — `6ed0c64c` (decisions log +3), `73dc99ed` (WORKLIST refresh), `a8cdd1df` (`eod_quotations_reconcile_batch_commit_v1.sh`), `809c792b` (Mac 18-file batch — quotations dir), `464f104b` (promote_v1), `48483679` (promote_v2), `3c396807` (promote_v3), `3e4c1b66` (sidecar master), `43b9cbe9` (page gen v1), `4ad11526` (page gen v2), `6918d6e0` (per-quote v1).
- Decisions log entry appended in this same EOD batch (§11x).

### 2026-06-19 AM-3 (v1.8.0 route_v2 GREEN + horizon backfill + Q1 admit-orphan triage RESOLVED)

- **`route_quotations_to_horizon_v2.py` SHIPPED** (commit `56cc4622`). Gotcha-9 strike #1 of session: route_v1 trusted the corpus's stored `headline_norm` field, but `union_article_corpus_v2.py` writes the live `normalize_text(headline)` form which diverges from the stored field on 220 articles (dashes/slashes/commas stripped vs preserved). v1 produced 217 P1 (wrong). v2 recomputes the corpus norm at routing time from each article's `headline` — produces 474 P1 / 230 routed initially. Post-backfill re-run yields **474/474 routed, 292 tuples, 220 scorable after prefilter_skip removal**. Tuple distribution: prefilter_skip=72, SH-3y=186, SH-5y=26, LH=8.
- **Horizon backfill SHIPPED** (commit `c38ff792`). 244 P1-eligible rows had blank `forecast_horizon_years` cells (a pure analyst-authoring gap, 0 parse failures, confirmed by `diag_route_v2_residuals_v1.py`). `backfill_blank_horizon_3y_v2.py` plugged `3` (SH-3y default) into all 244 cells with sidecar audit at `_horizon_backfill_3y_v1_applied.txt` (20,481 B, lists every (rid, headline) pair). Approach (c) chosen: sidecar audit only, no master CSV schema change. Gotcha-9 strike #2 of session: backfill_v1 assumed a `reject` column inherited from format-mismatch-review CSV context; `kastner_quotes_clean.csv` has NO reject column (18 cols only). FATAL on dry-run; v2 fixed.
- **Q1 admit-orphan triage RESOLVED** (commit `914bdd4c`). 10 rows persistently classified as admit-orphans (P1-eligible but no corpus article match after Gotcha-9 fix). `diag_admit_orphan_sources_v2.py` reads canonical schema (`raw_preview` + `headline_attempted` + `reason` for unclaimed segments; v1 used wrong keys — Gotcha-9 strike #3, produced 328-char empty blob and misclassified all 10 as D-class). v2 produced clean triage:
  - **A-class (recoverable PDF segment, 2 rows):** rid 494 "IBM Lands Navy Supercomputer Deal" + rid 1005 "Dell, Oracle & Linux: Your Next SAP Platform?". Both present in raw PDF segments — the boundary detector dropped them during substrate build. Backlog item raised (re-run substrate union with relaxed heuristics OR hand-author article stubs).
  - **E-class (cited in corpus body, 8 rows):** rids 199, 374, 426, 458, 780, 1020, 1045, 1181. Headlines cited inside OTHER corpus articles' bodies (mentioned in narrative; standalone article never archived). 7 of 8 also in raw RTF; rid 1181 "Sony Playstation" not even in RTF. Disposition: accept-as-cited; backlog item raised to document in substrate trust manifest.
  - **Critical: all 10 orphans scorable on quote text alone.** v1.8.0 scorer prompt does NOT require full article body — `kastner_quotation + immediate_context` is sufficient. Q1 NOT BLOCKING calibration; recovery deferred.
- **Substrate trinity confirmed INTACT** (false-alarm scare from diag_v1 bug). Pete verified via `ls -la` + `head`: `article_corpus_v1.json` 2,180,897 B (179 articles), `_pdf_segments_unclaimed_v1.json` 306,819 B (`total_segments: 546`), `quote_only_rows_v1.csv` 564,397 B (734 rows). No substrate shrinkage from union v2; `union_article_corpus_v2.py --commit` was NOT run by Pete. v1.8.0 substrate cap remains ~470-480 absent additional source PDFs (per F4 finding).
- **Three Gotcha-9 strikes in one session — producer/consumer contract verification is now empirically demonstrated as mandatory pre-commit.** Pattern across all three: docstring-claimed schema diverged from actual code. Lesson for v1.8.x and beyond: grep every consumer column reference against producer's actual write site; diff producer docstring against actual code; never trust schema documentation alone.
- **v1.8.0 state of play:** 220 scorable tuples (above 170 highlight-reel target), Q1 closed, calibration unblocked. Next session: ship `score_quotations_calibration_v1.py` A/B harness.
- **Decisions log entry drafted** at `/home/user/workspace/decisions_log_entry_2026_06_19_q1_orphan_triage_v1.md` (6,266 B). Append to `_decisions_log.md` at EOD (this session's batch commit).
- Scripts shipped this AM-3 segment: `route_quotations_to_horizon_v2.py` (`56cc4622`), `diag_route_v2_residuals_v1.py` (`0696b134`), `backfill_blank_horizon_3y_v2.py` (`c38ff792`), `diag_admit_orphan_sources_v2.py` (`914bdd4c`).

### 2026-06-19 AM (Quotations corpus scoping — v1.8.0)

- **v1.8.0 union substrate SHIPPED** (2026-06-19 AM). After PM-shipped `detect_article_boundaries_v2.py` only hit 18.7% headline-match and D6 diagnostic revealed three PDF format families (Computerworld working / Wired+ECT broken / Kastner Blog no-pattern), Pete chose path (b) — ship the working RTF+Computerworld subset as Pipeline 1, route everything else to Pipeline 2, and persist the unclaimed-PDF-segments salvage substrate so future re-extraction never re-parses the source PDF. Shipped `scripts/union_article_corpus_v1.py` at archive `39aabb46`. Three canonical artifacts now live in `kastner-author/quotations/` (also at archive `39aabb46`): `article_corpus_v1.json` (1.78 MB, 105 articles — 60 RTF + 45 net-new PDF), `_pdf_segments_unclaimed_v1.json` (398 KB, 425/546 salvage segments), `quote_only_rows_v1.csv` (540 KB, 712 Pipeline 2 rows with route reasons `not_in_pdf=274 / pdf_format_mismatch=435 / headline_too_short_to_probe=3`). Partition math GREEN: 375 Pipeline 1 rows + 712 Pipeline 2 rows = 1087 (matches CSV total). Substrate is frozen — Pipelines 1 and 2 read from it exclusively; v1.9.0+ extraction work reads `_pdf_segments_unclaimed_v1.json` directly. Schema-contract gotcha avoided pre-commit: verified `det_mod.assemble_articles(segments, classifications)` arg order + return keys (`body_text` not `body`; `page_range` → reconstructed `spans_segments`) against `detect_article_boundaries_v2.py` lines 417-482 (Gotcha 9 protocol).
- **Three publication formats catalogued in PDF (D6 finding, 2026-06-19 AM):** (1) Computerworld `Page: N MM/DD/YY` — detector works (~103 admitted); (2) Wired.com + E-Commerce Times — different format, detector extracts wrong headline (222 misclassified); (3) Kastner Blog — no `Page: N` pattern, dates in body (18 UNKNOWN). Plus 51 over-eager CONTINUATION misclassifications and 168 articles genuinely not in PDF (Reuters, USA Today, WSJ partial, Boston Globe). All routed to Pipeline 2 via `quote_only_rows_v1.csv` with the right reason.
- **Silent-loss discovery + apply SHIPPED** (2026-06-19 AM, post-substrate). After substrate ship Pete asked why 425 unclaimed PDF segments + 105 article corpus produced only +375 Pipeline 1 rows. Drafted `discover_unindexed_kastner_quotes_v2.py` (R1 medium-confidence headline-probe rule, 24-col output: `reject` + 18 canonical + 5 provenance). Reject-column semantics locked per Pete: `bool(row.get("reject","").strip())` — blank/whitespace = ADMIT, any non-space char = REJECT. Dry-run found 125 candidates; Pete reviewed in Numbers as v3 (`_unindexed_kastner_candidates_v3.csv`), marked 4 reject + 121 admit. `apply_unindexed_quotes_v3.py --commit` appended 121 rows to `kastner_quotes_clean.csv` (1087 → 1208). Backup `bak_apply_unindexed_v3_20260619T124028Z` written. Re-ran `union_article_corpus_v1.py`: article corpus 105 → **179** articles (60 RTF + 119 net-new PDF), unclaimed segments 425 → 328, Pipeline 1 375 → **464** rows, Pipeline 2 712 → **744** rows, total 1208 ✓. Pipeline 2 reasons post-apply: `not_in_pdf=301 / pdf_format_mismatch=437 / headline_too_short_to_probe=6`. Commits: discover_v1 `64f67364`, discover_v2 `4316f9d8`, apply_v3 `67d88731`.
- **F4 substrate-cap finding (2026-06-19 AM):** Pete asked to attack the 437 `pdf_format_mismatch` rows. Drafted `diagnose_pdf_format_mismatch_v4.py` (4-iteration drilldown, classifier across 7 failure modes F1-F6 + F0 splits). Bugs found and fixed along the way: v1 read wrong column name `partition_reason` (union writes `pipeline_route_reason` — Gotcha 9 bit again), v2 crashed on `None` subscript. v3 ran clean but only 3 of 437 were salvageable (concentration in F4=278 / F0=149). v4 added headline-probe fallback splitting F0 → F0a (headline-in-norm-gap, salvageable) vs F0b (headline-also-not-in-PDF, terminal). **Final partition:** F4 `not_in_pdf` = 278 (63.6%, terminal), F0b `headline_also_not_in_pdf` = 132 (30.2%, terminal), F0a `headline_in_unclaimed_norm_gap` = 17 (3.9%, salvageable), F6 `detector_saw_different_title` = 7 (1.6%, Pete-review), F3 `headline_in_body_detector_missed` = 2 (0.5%, salvageable), F1 `detector_grabbed_midsentence` = 1 (0.2%, salvageable). **Key finding: 410 of 437 mismatches are source-document gaps (PDF doesn't contain the article), not code gaps.** Substrate work is effectively done — Pipeline 1 cap is ~470-480 absent finding additional source PDFs (deferred workstream). 20 substrate-salvageable rows + 7 Pete-review rows = 27 rows for the next-step review CSV.
- Diagnostic commits: v1 `a87607d2`, v2 `4bad6c6d`, v3 `edf4864d`, v4 `28b9239c`.
- _accumulator: items completed today land here_

### 2026-06-18 PM-3 (v1.7.0 SHIPPED + EOD bookkeeping) — preserved from yesterday

### 2026-06-18 PM-3 (v1.7.0 Mac-side cutover + F3b discovery + SHIP)

- **v1.7.0 SHIPPED.** Tag `v1.7.0` at archive `bd819f4e97e36ad80959dba30b3113f5d8d1621e`. GitHub Release published 2026-06-18T22:37:52Z (release ID 341674192) at [releases/tag/v1.7.0](https://github.com/shorttack/aberdeen-group-archive/releases/tag/v1.7.0). Release body 89 lines covering schema-and-discipline scope, F2/F3/F3b/F6/F7 closure summary, full commit lineage, shape audit, prescience master distribution, and Mac-side cutover narrative.
- **Block 1 (git pull + script stage)** GREEN on Mac. `cd ~/Desktop/Archive/aberdeen-group-archive && git pull` landed 4 sandbox commits `0f5c9d71..577892d4` (architecture map + F2 + F3+F6+F7 batch + docs batch + PM-2 EOD). Scripts `add_row_class_to_prescience_scores_v1.py` and `retag_cloud_parse_fails_v1.py` copied to `~/Desktop/Archive/scripts/`.
- **Block 2 (F3 dry-run)** ABORTED with `ERROR: column 'row_class' already exists. Aborting.` Diagnosis required fetching the Mac master via `gh api repos/shorttack/aberdeen-group-archive/git/blobs/<sha>` (file is 13.8MB, exceeds the contents endpoint blob-size limit). Sandbox inspection of fetched master: 17,085 rows × **12 cols** (column already added on Mac from a prior session) but **8,645 rows had NULL `row_class`** (new Pass C runs landed after the original F3 backfill) and **4 rows had legacy value `prefilter`** instead of canonical `prefilter_skip`.
- **Pete's Q1/Q2/Q3 lock-in.** Q1: canonical enum value is `prefilter_skip`, not `prefilter` (rename the 4 stragglers). Q2: backfill all 8,645 NULL `row_class` rows tonight, do not defer. Q3: push through tonight, don't roll back to a different cutover strategy.
- **F3b — `backfill_row_class_v1.py` drafted** (215 LOC, dry-run default, `--commit` opt-in, `csv.QUOTE_ALL`, UTC-stamped backup before write, row-parity check, prints distribution). Logic: detect-and-rename the 4 legacy `prefilter` rows → `prefilter_skip`, then classify each NULL-row_class row from its existing `prescience_score` + `source_pass` + flag columns using the same classifier as F3. Sandbox simulation against the fetched master matched the expected delta exactly (4 renames + 8,645 backfills = 8,649 mutations). Committed at archive `730ac65f` via `gh api -X PUT contents/`.
- **Block 2b (F3b dry-run on Mac)** matched the sandbox simulation byte-for-byte. Block 3 (F3b `--commit`) applied 8,649 mutations cleanly on Mac. Backup `~/Desktop/Archive/archive_masters/_master_prescience_scores.csv.bak_backfill_row_class_20260618T222707Z` written before mutation.
- **Block 4 (Phase 1+2 rebuild + shape audit)** GREEN. Shape unchanged from v1.6.2: studies=1453, observations=23926, entities=3276, technologies=4361, decades=6, high_prescience_studies=865. Prescience master post-cutover: 17,085 rows × 12 cols, zero NULL `row_class`, distribution `scored=16708 / parse_fail=116 / prefilter_skip=8 / preseed_skip=253 / no_anchor=0 / pending=0` (the `no_anchor` and `pending` classes are reserved for future Pass C runs).
- **Phase 5 skipped** intentionally. Grep'd `wiki/` for the 3 changed `Perplexity_Only/` docs (PRESCIENCE_ARCHITECTURE.md, MASTERS_NOTES.md, RELEASE_NOTES_v1_7_0.md): zero matches. The `Perplexity_Only/` directory is agent-context only and lives outside the wiki embedding index. No re-embed work needed for v1.7.0.
- **Pre-commit `git status` surfaced bycatch.** Working-tree `_master_observations.csv` modified (mtime Jun 16 07:19, predates today): HEAD has 16 columns, Mac working tree has 30 columns. Same 23,927 rows. 14 unknown columns added. Unshipped Mac-local migration of unknown provenance. **DEFERRED per D3 — production master moves require preauthorization.** Also 5 terminal-typo junk files from broken `tee` redirects: `--wiki`, `012`, `echo`, `python3`, `=== ALL PHASES COMPLETE ===`. Deleted before commit.
- **Block 7 (clean staging) → Block 8 (commit + push).** Staged only F3b output (`_master_prescience_scores.csv` + `archive_masters_pre_backfill_row_class_20260618T222707Z/_master_prescience_scores.csv`). Commit `bd819f4e` written with heredoc message; pushed past two branch-protection bypass warnings: "Pull requests must be created and approved" and "Commits must have verified signatures." Push succeeded (admin override).
- **GitHub Release published** via `gh api POST repos/.../releases` with tag `v1.7.0`, target `bd819f4e`, name "Multi-Horizon Prescience: row_class discipline + cloud parse-fail retag", body 89 lines.
- **Editing gotcha confirmed** (carry-forward): multi-edit batches with long blocks fail silently in the sandbox `edit` tool. Workaround used throughout PM-3: split into single edits or 2-3 small edits per call.
- **Carry-forward to next session** (also captured as Next up items): `_master_observations.csv` 30-col migration audit, skill update for detect-and-backfill mode, `datetime.utcnow()` deprecation in F3b template, junk file cleanup hardening, commit signing setup, untracked work catalog (Perplexity_Only/ SH outputs + scripts/ qwen audits + scripts/v3_obsolete/).

### 2026-06-18 PM-2 (v1.7.0 ship-gate cleanup — F2/F3/F6/F7 all CLOSED in sandbox)

- **F2 — promote_pass_c_to_master_v1.py byte-aligned to Mac §11v patch.** Pete pasted the 211-line Mac active copy (containing the 4-line provenance comment + 2-line `scorer_version`/`source_pass` defaults at lines 151-156); repo copy at `scripts/promote_pass_c_to_master_v1.py` was the stale pre-patch 207-line version. Overwrite committed at archive `7935aec7` via `gh api -X PUT contents/`. Sandbox ↔ Mac byte-identity restored.
- **F3 — `add_row_class_to_prescience_scores_v1.py` drafted** (251 LOC, dry-run default, `--commit` opt-in, `csv.QUOTE_ALL`, UTC-stamped backup before write, row-parity check, prints distribution). Adds 12th column `row_class` to `_master_prescience_scores.csv` with enum `{scored, parse_fail, prefilter_skip, preseed_skip, no_anchor, pending}`. Classifier logic: derives class from existing `prescience_score` + `source_pass` + flag columns (never from a free-text inference). Expected pre-cutover distribution (predicted from Pass C state on Mac): scored=8119, parse_fail=64, prefilter_skip=4, preseed_skip=253, no_anchor=0, pending=0, total=8440. Pete's decision: F3=A (add the column, all six classes modeled).
- **F6 — `retag_cloud_parse_fails_v1.py` drafted** (154 LOC, same invariants as F3). Surgical 12-row touch: existing 12 rows with `source_pass=pass_c_cloud` AND `prescience_score=-1` (the parse-failure sentinel) get retagged to `source_pass=pass_c_cloud_parse_fail`. `parse_ok` cloud rows unchanged. **MUST run AFTER F3** so `row_class` is already set; F6 leaves `row_class=parse_fail` for these 12 rows but disambiguates the SH from cloud parse-fail bucket. Pete's decision: F6=A.
- **F7 — `F7_preseed_skip_sh_treatment_decision_v1.md` drafted** (89 LOC decision doc, no script needed in v1.7.0). Locks the SH driver v8 semantics: preseed-skip rows score normally under SH (preseed is long-horizon-only, the 5y window doesn't trigger it); SH verdicts run in parallel to LH verdicts; **Rule A SH rollup** = `AVG(CASE WHEN prescience_score>=0 THEN prescience_score END) WHERE source_pass IN ('pass_c_sh_3y','pass_c_sh_5y') AND prescience_score>=0 GROUP BY study_id`, thresholds 3.5/2.0, empty→not-applicable. New SH source_pass taxonomy: `pass_c_sh_3y` / `pass_c_sh_5y` / `pass_c_sh_parse_fail`. Pete's decision: F7=A.
- **F3+F6+F7 batch commit** at archive `5f945dd9` via Git Data API (blob → tree → commit → ref). Three new files: `scripts/add_row_class_to_prescience_scores_v1.py`, `scripts/retag_cloud_parse_fails_v1.py`, `Perplexity_Only/F7_preseed_skip_sh_treatment_decision_v1.md`.
- **Docs batch commit** at archive `a6c7a007` via Git Data API. Three files: `Perplexity_Only/PRESCIENCE_ARCHITECTURE.md` (rev2: status header bumped, §1.3 promote script CLOSED `7935aec7`, §2.1 schema 11→12 cols + row_class enum table all 6 classes modeled, §7 cleanup map with closure commits, §9 rev2 changelog), `Perplexity_Only/MASTERS_NOTES.md` (v3: title v2→v3, support-file row 3,761/11 → 8,440/12, new 12-col schema entry for `_master_prescience_scores.csv`, preseed_b note extended with row_class cross-ref, new SH conventions block, v3 changelog), `Perplexity_Only/RELEASE_NOTES_v1_7_0.md` (NEW, 205 lines, four-finding closure narrative + Mac-side cutover runbook + verification checklists).
- **Editing gotcha discovered (sandbox-only):** multi-edit batches with long blocks fail silently in the sandbox `edit` tool. Workaround used throughout this segment: split into single edits or 2-3 small edits per call. Worth folding into agent operating procedure if it persists across sessions.
- **v1.7.0 ship gate state at PM-2 close:** four findings closed in sandbox. Mac-side cutover (F3+F6 dry-run → commit → Phase 1+2 → shape audit → surgical Phase 5 re-embed of the 3 changed `Perplexity_Only/` docs) deferred to next session per the Mac-side runbook in `Perplexity_Only/RELEASE_NOTES_v1_7_0.md`.

### 2026-06-18 PM (methodology-demo v2.0 + Phase 6 hardening + §11v D6 architecture map)

- **#1 Methodology-demo v2.0 regen** shipped as new slug `2026-kastner-prescience-methodology-demo-v2-0cdf49` (one-and-done, not in-place patch — Pete: *"the language will be harder to patch than generate new"*). 4 commits across both repos:
  - archive `0a88d455` (source markdown + `add_methodology_demo_v2_study_row_v1.py` D3-preauthorized masters row 1452→1453)
  - wiki `49f33d3f` (hand-authored v2.0 study page + patched `RELEASE_NOTES_v1_6_2.md`)
  - archive `53fc748c` (`reembed_single_page_v1.py` — surgical fix for Phase 5 race condition; DuckDB COPY + atomic rename + row-parity check)
  - archive `229843c4` (`06_emit_scaffolding_v5.py` — .gitignore template hardened + qwen3.5 fallback corrected)
  - kw ask retrieves v2.0 at top (score 0.499). Phase 3 wall-clock ~6 hr on tier-1 LLM.
- **#4 Phase 6 v5** (partial §4 closure): .gitignore template embedded as 17-line content with warning comment; subsequent reruns no longer clobber. LOCAL_MODEL fallback stale-string `qwen3.6:27b-mlx` corrected to `qwen3.5:27b-mlx`. README/AGENTS/chat-starter vocabulary refresh still pending.
- **§11v D6 PRESCIENCE ARCHITECTURE AUDIT** — architecture MAP shipped at `Perplexity_Only/PRESCIENCE_ARCHITECTURE.md` (archive commit `0f5c9d71`, 376 lines, 23,672 bytes). Companion to the existing 2026-06-15 findings report. v1.7.0 ship gate now reduced to 4 must-fix items (F2, F3, F6, F7).
- **Gotcha 13 candidate identified**: Phase 5 reads its file list at start; mid-flight `git pull` races overwrite mtime-newer pages but Phase 5 still embeds the pre-pull content. Surgical re-embed script handles single-page case. Should fold into `kastner-archive-pipeline` skill next session.
- **Decisions log entry written**: `decisions_log_entry_2026_06_18_methodology_demo_v2_and_prescience_arch_v1.md` (4 decisions + shape audit + commits table + open follow-ups). Mac-side uncommitted: scaffolding regen + indices + .gitignore + embeddings.parquet to wiki repo, deferred to next session.

### 2026-06-18 AM (post-v1.6.2 cleanup)

- **#2 `.gitignore` patches** shipped: archive `5c3f2f2d` (added `*.bak_*`), wiki `9df154f4` (added `__pycache__/`+`*.pyc`+`*.pyo`+`*.bak_*` AND removed the bycatch .bak file in same commit). Mac pulled clean both repos.

### 2026-06-17 (v1.6.2 release ship — see decisions_log_entry_2026_06_17_v1_6_2_release_v1.md)

- Phase 1+2 commit train shipped (a1661603, 27a2f9d7, b2c45f39, 40d5bc9c, 7e85eaee).
- Phase 3 ran 7h on qwen3.5:27b-mlx (after killing qwen3.6 pin mismatch; PID 20950 killed, `ollama rm qwen3.6:27b-mlx`). 10,382 pages emitted.
- Phase 4/5/6 complete; methodology-demo prose sed-patched in place (deferred full regen → today's #1).
- Archive repo: commit `a472cc4f`, tag `v1.6.2` shipped via gh API tree-commit (RELEASE_NOTES_v1_6_2.md + new README.md).
- Staging commit `6cd3e946` placed wiki files at `staging/v1_6_2/` so Pete could `cp` instead of heredoc (heredoc bit us at 09:30 — corrupted 3 Phase scripts; recovered via `cp` from clone).
- Wiki repo: commit `bc71a2a7`, tag `v1.6.2` pushed from Mac. 4,740 files changed, 39,725 ins / 39,551 del, 64.72 MiB. LFS warning on `data/embeddings.parquet` (63.41 MB) → today's #3. Bycatch `.bak_*` file committed → today's #2.
- Verdict distribution locked: high 498 / medium 330 / low 276 / n/a 346 / [DEFERRED] 1 / NULL 1. Wikilinks: 5,597 study→entity / 11,050 study→tech.
- qwen3.6 evaluated and removed from stack via local-model-upgrade-gates Gate 0 read of OLLAMA_GOTCHAS.md.
