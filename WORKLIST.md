...
**Last updated:** 2026-06-27 EOD (PC-Deals -mx v2 Pass C scored + 13 verdicts written; scores master 17085→17251; v1-vs-v2 report delivered; Phase 1/2 rebuild carried to next session)
...
### Mac MCP Bridge Phase 0 scaffolding + backlog triage (2026-06-24 PM)

- **Mac MCP Bridge Phase 0 scaffold created** — 11-file FastMCP read-only scaffold generated for `~/Repos/mac_mcp_bridge/` (`pyproject.toml`, README, config/server package, connector template, smoke tests). Runtime scope remains Phase 0 only: diagnostic `bridge_info`, no DuckDB tools yet, no mutation tools.
- **Local test fix applied** — patched macOS `/tmp` path normalization assertions and FastMCP v2 `get_tool()` registration check. Pete ran `pytest -q` in `~/Repos/mac_mcp_bridge`: **8 passed in 0.32s**.
- **Mac MCP Bridge Phase 1 DuckDB tools shipped locally** — applied `/tmp/mac_mcp_bridge_phase1.tgz` to `~/Repos/mac_mcp_bridge` via `/tmp/apply_mac_mcp_phase1.sh`; backup at `/tmp/mac_mcp_bridge_backup_20260624T134134/mac_mcp_bridge`. Added read-only `duckdb_query`, `duckdb_tables`, `duckdb_describe`, `duckdb>=1.0.0`, README/tool docs, and temp-DB tests. Pete ran installer/test suite on Mac: **61 passed in 1.01s**, `PHASE1_OK`.
- **Mac MCP Bridge Phase 1 real-DB smoke test passed** — ran through `kastner_archive_mcp.duckdb_tools` against `/Users/scott/Repos/kastner-aberdeen-wiki/db/kastner.duckdb` with read-only connection. `v_studies` count returned **1454**; sample `v_studies` rows, `list_tables()`, and `describe("v_studies")` all returned JSON-serializable rows.
- **archive_masters retirement Phase A+B verified** — tracked zero-byte bycatch restored via `git restore`; repo tracked status clean before edits. Moved `archive_masters/_master_player_rebuttals.csv` → repo root `_master_player_rebuttals.csv`, mirrored to wiki root `/Users/scott/Repos/kastner-aberdeen-wiki/_master_player_rebuttals.csv`, and patched active scripts for repo-root masters. Reconciled drifted `_known_entities.csv` and `_known_technologies.csv` from old canonical to repo root. Phase B patched remaining active absolute-root references, then renamed `~/Desktop/Archive/archive_masters` → `~/Desktop/Archive/_retired_archive_masters_20260624T184237Z` (rename only, no delete). Pete reran Phase 1+2 from repo-root masters GREEN. Final shape stable: 1454 studies / 23990 observations / 3291 entities / 4370 technologies / 1297 codes / 865 high-prescience; known sidecars 3300 entities / 4371 technologies; `PHASE_B_VERIFY_OK`.
- **Player rebuttal wiki integration verified** — patched `scripts/build/03_generate_vault_v2.py` so Phase 3 can load `_master_player_rebuttals.csv` from archive repo root or wiki-root mirror and render a deterministic `## Player rebuttals` section on generated study pages. Migrated the mis-bound DECtp wiki note from `rebuttal-debit-credit-ibm-dec-1988-2-2fc147-2026-06-13.md` to `rebuttal-dectp-press-conference-transcript-and-benchmark-charts-plaza-5e5836-2026-06-13.md`; fixed `slug`, `rebuts_study`, `related_studies`, tag binding, and subject-title text. Regenerated the Plaza study page only; section now links to the wiki note and source note path. Follow-up fixed the targeted page back to `tier: 1`. Final checks: wiki `git diff --check` OK, archive `git diff --check` OK, `03_generate_vault_v2.py` compiles, old `archive_masters` absolute-path references clear.
- **EOD commits + local bycatch cleanup complete** — archive repo pushed `ba301179` (`Retire standalone archive_masters`); wiki repo pushed `1f1f1ced` (`Surface DECtp player rebuttal in wiki`). `kw pending` clean after commit. Untracked bycatch from archive and wiki working trees moved, not deleted, to `/Users/scott/Desktop/Archive/_repo_bycatch_20260624T194737Z` (74 files total). Final `git status --short` clean in both repos.

### Mac MCP Bridge Phase 2 — remote connector LIVE (2026-06-26 / 2026-06-27)

- **Root cause of original connector failure found + fixed** — the stdio connector JSON pointed at a DEAD pyenv interpreter (`~/.pyenv/shims/python3`); corrected to the project venv `/.venv/bin/python`. stdio bridge then verified end-to-end (`bridge_info`, `duckdb_tables`, `duckdb_query` → 1454).
- **Established Perplexity custom connectors are REMOTE HTTPS ONLY** — no stdio/local-command option for this account; stdio mode is unusable for the Computer connector. Converted the bridge to an authenticated HTTP transport.
- **HTTP transport added (stdio retained)** — `server.py` now transport-aware via `BRIDGE_TRANSPORT` (stdio default | http). New `http_auth.py` Bearer/API-Key middleware accepts `Authorization: Bearer <t>`, raw `Authorization: <t>`, `X-API-Key`, `Api-Key`, `X-Api-Token`; constant-time compare; non-`/mcp` paths bypass auth for health probes. `run_http.sh` auto-generates the token at `~/.config/kastner_mcp/token`. 61 tests pass; in-process Starlette TestClient verified no-token→401, wrong→403, correct→200.
- **Permanent public URL via ngrok reserved domain** — chose ngrok over Cloudflare (no domain needed). Reserved domain `dolphin-washer-slush.ngrok-free.dev` → `http://127.0.0.1:8765`. Key gotcha: upstream MUST be `http://127.0.0.1:8765` not `localhost` (localhost upstream returned 502 through the tunnel).
- **launchd auto-start + hardening** — service `com.kastner.ngrok.bridge` runs a wrapper `~/Repos/mac_mcp_bridge/start_ngrok_bridge.sh` that pkills strays + sleeps 6s before `exec ngrok start kastner-bridge` (defeats `ERR_NGROK_334` endpoint-already-online on boot). Plist adds `RunAtLoad`, `KeepAlive`, `ThrottleInterval 10`, `ProcessType Background`, explicit PATH. `plutil -lint` OK. Verified TUNNEL HTTP 200 + `bridge_info` + `v_studies`=1454 through the permanent URL from chat.
- **Connector registered + cleanup** — custom connector "Perplexity bridge v2" registered at `https://dolphin-washer-slush.ngrok-free.dev/mcp` (API-Key auth, Streamable HTTP), CONNECTED and verified in chat. Stale first attempt "Perplexity Bridge to Mac" (old cloudflared URL) deleted by Pete. GitHub: per Pete's instruction NOT touched — bridge code remains local-only on the Mac; empty `shorttack/mac_mcp_bridge` left as-is.
- **Files written to Mac (uncommitted, local only):** `src/kastner_archive_mcp/server.py` (transport-aware), `src/kastner_archive_mcp/http_auth.py`, `run_http.sh`, `start_ngrok_bridge.sh`, `docs/REMOTE_CONNECTOR.md`, `~/Library/LaunchAgents/com.kastner.ngrok.bridge.plist`, ngrok config `~/Library/Application Support/ngrok/ngrok.yml` (endpoint `kastner-bridge`).
...
**Mac data state (CSV masters, post-`-mx` v2, 2026-06-27 EOD, DuckDB NOT yet rebuilt):** studies 1504 · observations 24715 · prescience-scores 17251 (+166 `-mx`) · 13 `-mx` verdicts written (4 med→high, 3 high→med). Live DuckDB still pre-`-mx` until Phase 1/2 runs.


### Done this session — 2026-06-27 EOD: PC-Deals -mx v2 Pass C + verdicts (2026-06-27 EOD)

- **Scored 13 PC-Deals `-mx` (v2 model-extraction) studies through Pass C v7.** Built + shipped `run_prescience_pass_c_v7.py` (line-62 path fix over the stale v6-copy that sat in the repo as "v7"), `build_mx_pass_c_manifest_v1.py` (166 scorable obs across 13 studies; 559 obs in 37 not-applicable studies skipped). Full batch: 166 obs scored, 162 parse_ok, 4 parse-fail. Scores at `pass_c_v7_mx_tier.csv`. Score dist: `-1:4, 0:89, 1:3, 2:8, 3:25, 4:32, 5:5`.
- **Locked the verdict rule (2026-06-27):** `used = integer scores 1..5 only` — EXCLUDES 0 (= "cannot assess" per rubric, NOT a failed prediction), -1 (parse-fail), blanks. **No min-count gate** (Pete: weekly PC Deals can't be held to a high obs floor without "setting up for failure"). `mean>=3.5 high; >=2.0 medium; 0<mean<2.0 low; no assessable obs → not-applicable`. Every rationale discloses its denominator (n assessable / n market-data-zero / n parse-fail). This supersedes Rule A (`used = scores != -1`) for the `-mx` tier: Rule A counted 0s; the new rule excludes them.
- **Promoted 166 `-mx` scores into the prescience master.** `promote_mx_to_master_v2.py` (12-col-aware; preserves `row_class`, which the legacy 11-col `promote_pass_c_to_master_v1.py` would have dropped). `_master_prescience_scores.csv`: 17085 → **17251** rows, 0 dupes, row_class {scored:162, parse_fail:4}. Backup written.
- **Wrote 13 study verdicts into `_master_studies.csv`.** `write_mx_verdicts_to_studies_v1.py` — computed enum into `prescience`; AUTHORED `prescience_rationale` preserved verbatim with a dated Pass C v7 audit note APPENDED (idempotency guard on the date string). **7 of 13 enums changed:** 4 medium→high (business-2003-03-17, weekly-2002-10-27, weekly-2002-11-03), 3 high→medium (intel-processor-prices, pc-replacement-insight1, weekly-2002-11-14-p4-ht), 6 unchanged. Backup written. Read-back parity PASS.
- **v1-vs-v2 comparison report drafted** (`scripts/compute_mx_verdicts_v1.py` + analysis; report delivered in-chat). Three findings: (1) **coverage win** — v2 produced first-ever Pass C verdicts for 8 of 13 studies (the weeklies + business-deals) the A-side never scored; (2) **false-positive correction** — v1 rated all 5 comparable studies "high" by scoring market-data facts as predictions; v2's typed extraction parks them at 0 and corrects 3 of 5 to medium; (3) **prediction-yield increase** — v2's assessable base grows 1.25–3.5× per study (why-aberdeen 5→13 real predictions against 12→1 market-data zeros). Headline: v2 trades inflated highs for honest mediums while minting new verdicts elsewhere — quality up in both directions.
- **Architecture note: sync step is obsolete.** Since the masters moved to repo root (2026-06-24), `sync_studies_verdicts_repo_from_archive_masters_v2.py` is dead — its source path `~/Desktop/Archive/archive_masters/_master_studies.csv` no longer exists. The verdict-write script writes the canonical+repo copy directly; no sync needed.

### Carry-forward (next session — FIRST action)

- **Phase 1/2 rebuild has NOT run.** The live DuckDB still reflects the pre-`-mx` state. Next session, FIRST action: `01_load_csvs_v2.py --archive ~/Desktop/Archive/aberdeen-group-archive --wiki ~/Repos/kastner-aberdeen-wiki` → `02_build_data_layer_v4.py --wiki ~/Repos/kastner-aberdeen-wiki`. Run the **shape audit before AND after**. Then Phases 3-6 + re-embed (Phase 5 ~14-15 min) so `kw ask` reflects the new `-mx` studies + verdicts. NOTE: use `--archive ~/Desktop/Archive/aberdeen-group-archive` (NOT the retired `archive_masters/` path some skill docs still show).
- **Backlog (per Pete):** full unscored Pass C sweep (~4,514 pre-existing obs never scored). Deferred.
