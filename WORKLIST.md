...
**Last updated:** 2026-06-13 AM (§11u-cont Pass B Completion Commit + Release v1.6.1. Phase 3 done overnight (10,382 pages: 1452 studies / 3276 entities / 4361 technologies / 1293 codes). Phase 4 done (decades, collections, codes index, 5 bases). Phase 5 running now (10,437 page embeddings via bge-m3). Phase 6 next. Today: finish Phases 5+6, run shape audit, ship Pass B Completion Commit (4 Perplexity_Only files + masters + decisions log v2 + WORKLIST), cut Release v1.6.1 for Zenodo, then seed `Pete_Only/` directory.)
**Current ship state:** archive `origin/main` at `0391dabf` (2026-06-12 PM apply_passb_transcripts_v2.py commit, parent `0ab3be59` = original Pass B batch). Yesterday's PM sandbox edits (Perplexity_Only banner + Pete_Only next-up + 4 Perplexity_Only/ helper files + MASTERS_NOTES v2 + RELEASE_NOTES_v1.6.1 + decisions log entry v2) NOT YET pushed — will ship as today's Pass B Completion Commit after Phase 6 + shape audit pass. Mac state post-Phase 1+2: **1452 studies / 23926 observations / 3276 entities / 4361 techs / 3876 entity-study pairs / 5375 tech-study pairs**. `v_studies_with_high_prescience` = 124. `decades_covered` = 6 (using `//` integer division). Pass B obs canonical-ID rate: 295/295 = 100%. Wiki `origin/main` unchanged from v1.6 release — will bump via Release v1.6.1 today. bge-m3:latest canonical embedding model. **Security:** git `user.name` was "Catalina" (one of Pete's passwords); leak found 2026-06-01 PM, rotated to `shorttack`; 976 historical commits across the two public repos retain dead-string "Catalina" in Author metadata.
...
- [ ] **§11v Skill maintenance** — update `kastner-archive-pipeline` to reflect `~/Repos/kastner-aberdeen-wiki/` (not `~/Desktop/kastner_wiki/`) as the canonical wiki path; fix shape-audit SQL to use `pub_year // 10` instead of `pub_year / 10` (DOUBLE bug, 38 decade buckets); document `v_studies_by_decade` as the canonical decade-count view.
