...
### 2026-06-22 (CompChem ingest + v1.9.0 release)

- **CompChem 1989 ingested** — 175-file study package committed at archive `a02c23f1` + 8.5MB PDF at private `33a52bf3`. New top-level `project_examples/` path established. 64 observations extracted (no limit per Pete's directive). Recovers one of three §1 missing-sources registry items.
- **v1.9.0 release shipped on both repos** — archive `v1.9.0` + wiki `v1.9.0` tagged, pushed, and GitHub Releases published with combined notes covering v1.8.0 substrate silent-loss work + CompChem exemplar. Skipped untagged v1.8.0 version number.
- **Decisions log entry** — appended 2026-06-22 entry for CompChem + v1.9.0 (in EOD batch).
- **Pre-staged release notes commits** — `RELEASE_NOTES_v1_9_0.md` to archive (`71b8a385`) + wiki (`e018d1f1`) sibling.
- **Gotchas reinforced**: Apostrophes in Python heredoc → write to separate `.py` file (CompChem observations CSV generation); `csv.QUOTE_ALL` mandatory; plain-text gate false positive when header >200 bytes (verify manually); GitHub URLs in messages → use `api_credentials=["github"]` connector, not browser_task.
