...
description: >
  ...
  study. Supports six Kastner collection types: video transcripts, memoirs,
  employer records, AI responses, technology topics, and DCT. v19 adds Pass
  A structural verification, wiki propagation, schema normalization, and
  master regen integrity checks. v20 adds the obs_id Universal Normalizer
  (13-bucket classifier-driven repair of legacy observation IDs) and the
  legacy_obs_id audit column on _master_observations.csv (17 cols).
metadata:
  version: '20'
...
| `observations.csv` (per-study) | 12 | obs_id, study_id, entity_id, tech_id, observation_type, year_observed, metric_name, metric_value, confidence, methodology_code, source_page, notes |
| `_master_observations.csv` (v20) | 17 | obs_id, study_id, entity_id, tech_id, observation_type, year_observed, metric_name, metric_value, confidence, verification_method, methodology_code, source_page, notes, collection, thread_tag, section, legacy_obs_id |
...
### 19. Schema Normalization Playbook (v19, extended in v20)

Legacy or externally contributed `observations.csv` files often drift from
the canonical 12-column per-study schema. v19 codifies a deterministic
normalization pass that runs *before* the CSV Validation Gate (Section 16).
v20 documents the master-level superset emitted by Pass A + the obs_id
Universal Normalizer (Section 21).

#### 19.1 Canonical Schemas (Reference)

**Per-study `observations.csv` — 12 columns** (unchanged since v17):
...
**`_master_observations.csv` — 17 columns** (v20):
...
The master accumulates four columns beyond the per-study schema:

| column | added by | purpose |
|---|---|---|
| `verification_method` | Pass A (Section 17) | Sourcing classifier for every row. |
| `collection` | Master regen | Inherited from the parent study's collection (`dct`, `memoir`, etc.). |
| `thread_tag` | Schema normalization (Section 19.4) | Promoted from a legacy column so Obsidian Dataview queries can filter by thread. |
| `section` | Master regen | Free-text section/heading reference from the source document. |
| `legacy_obs_id` | obs_id Universal Normalizer (Section 21) | Audit-trail capture of the pre-normalization obs_id. Empty for rows that were already canonical. |
...
**v20 reference state (post obs_id Universal Normalizer, 2026-05-24):**

- 23,605 rows in `_master_observations.csv` × 17 columns
- 100% canonical obs_id format: 23,594 `STANDARD` + 11 `STANDARD_LETTER_SUFFIX`
- 0 within-study duplicates, 0 cross-study collisions
- 3,844 rows carry a non-empty `legacy_obs_id` (audit trail of the v3 normalizer rewrite)
- Full pre-fix backup at `archive_masters_pre_fix_v3_backup/20260524_141559/` (18 files)
...
### 21. obs_id Universal Normalizer (v20)

Legacy and cross-collection master regens accumulate observation IDs in
multiple shapes: study-prefixed standards, bare numeric IDs, typed-prefix
DCT shapes, letter-suffixed clones, concatenated typed+standard hybrids,
missing-`OBS-` separator variants, and more. The v20 normalizer collapses
all of these into a single canonical shape in one pass, capturing the
original value in a new `legacy_obs_id` audit column.

#### 21.1 Canonical obs_id Format

```
{study_id}-OBS-{NNN}              ← STANDARD (e.g., 2003-acme-pricing-a1b2c3-OBS-007)
{study_id}-OBS-{NNN}{letter}      ← STANDARD_LETTER_SUFFIX (rare; preserved as-is)
```

All other shapes are non-canonical and MUST be rewritten by the
normalizer before being committed to the master.

#### 21.2 Bucket Catalog (13 buckets, 100% coverage)

The universal classifier in `diagnose_all_obs_patterns_v2.py` assigns
every row to exactly one bucket:

| bucket | shape | action |
|---|---|---|
| `STANDARD` | `{study_id}-OBS-NNN` | Leave unchanged. |
| `STANDARD_LETTER_SUFFIX` | `{study_id}-OBS-NNNa` | Leave unchanged. |
| `BARE_NUMERIC` | `NNN` | Prepend `{study_id}-OBS-`. |
| `T_PREFIX_2026` | `T-NNN` / `T-NNNNN` (DCT typed) | Map to `{study_id}-OBS-NNN`. |
| `S_PREFIX` | `S-NNN` | Map to `{study_id}-OBS-NNN`. |
| `D_PREFIX` | `D-NNN` | Map to `{study_id}-OBS-NNN`. |
| `OBS_PREFIX_NO_STUDY` | `OBS-NNN` | Prepend `{study_id}-`. |
| `SHORT_COHORT` | `{cohort}-NNN` (short non-study prefix) | Map to `{study_id}-OBS-NNN`. |
| `MISSING_OBS_SEPARATOR` | `{study_id}-NNN` | Insert `-OBS` between stem and number. |
| `MISSING_OBS_PREFIX_MISMATCH` | `{wrong_study_id}-OBS-NNN` | Replace stem with the row's actual `study_id`. |
| `CONCATENATED_TYPED_STANDARD` | `T-NNN-{study_id}-OBS-NNN` | Strip the typed prefix; keep the canonical tail. |
| `LETTER_SUFFIX` | `{study_id}-OBS-NNNa` variants outside the canonical letter set | Normalize letter case; if collision, allocate a fresh sequence number. |
| `UNCLASSIFIED` | none after v2 classifier | If ever encountered, halt and extend the classifier. |

#### 21.3 Allocation Strategy

The normalizer uses **FRESH allocation per study**: when rewriting a row,
it scans the study's existing canonical `OBS-NNN` numbers and assigns the
next free sequence number. This guarantees:

- No within-study collisions.
- No cross-study leakage (each study has its own counter).
- Letter suffixes (`STANDARD_LETTER_SUFFIX`) are NEVER renumbered — they
  are treated as already canonical.

APPEND mode (rewrite at end of study's number range) is supported for
backfill scenarios but is not the default. FRESH is the default.

#### 21.4 legacy_obs_id Audit Column

For every row the normalizer rewrites, the **original** obs_id value is
written to a new column `legacy_obs_id`. Rows that were already canonical
get an empty string in that column. This:

- Preserves the forever-archive principle (no information is destroyed).
- Lets downstream consumers grep for `legacy_obs_id != ''` to find any
  rows that experienced a rewrite.
- Is safe for the Wiki and DuckDB layers — they `SELECT obs_id` and do
  not depend on the legacy value, but the column is available if needed.

#### 21.5 Invocation

```bash
# Audit only — preview rewrites with no file changes.
python fix_v2_residuals_v3.py --audit-only --archive /Users/scott/Desktop/Archive/

# Live run — applies rewrites, creates timestamped backup of masters first.
python fix_v2_residuals_v3.py --archive /Users/scott/Desktop/Archive/
```

The script:

1. Loads `_master_observations.csv`.
2. Classifies every row using the 13-bucket classifier.
3. Plans rewrites per bucket; checks for within-study collisions.
4. Writes a full timestamped backup of all 18 master files.
5. Applies the rewrites with `csv.QUOTE_ALL` (Section 16.2 Check 4).
6. Verifies that 100% of rows are `STANDARD` or `STANDARD_LETTER_SUFFIX`
   after the rewrite, and that no `obs_id` is duplicated within a study.
7. Logs every rewrite to `_obs_id_rewrite_log.csv` (timestamp, study_id,
   bucket, legacy_obs_id, new_obs_id).

#### 21.6 Wiki & SQL Safety

Before running the normalizer on a live archive, audit downstream:

- **DuckDB views** (Section 18.3): use only `obs_id` as a key for COUNT
  and JOIN; the column rename is invisible to view definitions.
- **Obsidian wiki** (Section 18): wikilinks resolve by `entity_id`,
  `tech_id`, and `study_id` — NOT by `obs_id`. Dataview queries reference
  observation rows by frontmatter fields, not by ID.
- **Refresh data layer**: re-run `refresh_data_layer.py` after the
  normalizer to rebuild Parquet + DuckDB with the new IDs. The added
  `legacy_obs_id` column appears in `observations.parquet` automatically.

The normalizer is therefore safe to run before wiki Phase 2 surgical
update (Section 18.2).

#### 21.7 Reproducible Artifacts

| artifact | purpose |
|---|---|
| `diagnose_all_obs_patterns_v2.py` | Universal 13-bucket classifier; run first to understand corpus state. |
| `fix_v2_residuals_v3.py` | The normalizer itself; idempotent. |
| `fix_v2_residuals_v3_README.md` | Step-by-step operator guide. |
| `_obs_id_rewrite_log.csv` | Per-row rewrite log (study_id, bucket, legacy_obs_id, new_obs_id). |
| `archive_masters_pre_fix_v3_backup/{timestamp}/` | Full master-file backup created by every live run. |
