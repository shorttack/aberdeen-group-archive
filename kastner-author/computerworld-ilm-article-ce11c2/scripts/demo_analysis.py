...
Frictionless Data Package: computerworld-ilm-article-ce11c2

Waiting for ILM?
Peter S. Kastner, July 2004
"""
...
def demo_analysis(root):
    ...
    entities = load_csv(root, "entities.csv")
    technologies = load_csv(root, "technologies.csv")
    entity_map = {e["entity_id"]: e["entity_name"] for e in entities}
    tech_map = {t["tech_id"]: t["tech_name"] for t in technologies}

    # Six-step ILM framework
    framework = [r for r in observations if r["observation_type"] == "framework-factor"]
    print(f"\n  Six-Step ILM Recipe: {len(framework)} steps")
    for f_obs in framework:
        print(f"    {f_obs['obs_id']}: {f_obs['metric_name']} -- {f_obs['metric_value'][:80]}...")

    # Technology assessments
    tech_assessments = [r for r in observations if r["observation_type"] == "technology-assessment"]
    print(f"\n  Technology Assessments: {len(tech_assessments)}")
    for ta in tech_assessments:
        tech_name = tech_map.get(ta["tech_id"], ta["tech_id"] or "(general)")
        print(f"    {ta['obs_id']}: {tech_name} -- {ta['metric_value'][:80]}...")

    # Market data
    market = [r for r in observations if r["observation_type"] == "market-data"]
    print(f"\n  Market Data Points: {len(market)}")
    for m in market:
        print(f"    {m['obs_id']}: {m['metric_name']} = {m['metric_value']}")
    ...
    for p in predictions:
        tech_name = tech_map.get(p["tech_id"], p["tech_id"] or "(general)")
        print(f"    {p['obs_id']}: {tech_name} -- {p['metric_value'][:80]}...")

    print(f"\n  Actual Outcomes: {len(outcomes)}")
    for o in outcomes:
        tech_name = tech_map.get(o["tech_id"], o["tech_id"] or "(general)")
        print(f"    {o['obs_id']}: {tech_name} [{o['year_observed']}] -- {o['metric_value'][:80]}...")

    # Strategy classifications
    strategies = [r for r in observations if r["observation_type"] == "strategy-classification"]
    print(f"\n  Strategy Classifications: {len(strategies)}")
    for s in strategies:
        print(f"    {s['obs_id']}: {s['metric_name']} -- {s['metric_value'][:80]}...")

    # Expert opinions
    expert = [r for r in observations if r["observation_type"] == "expert-opinion"]
    print(f"\n  Expert Opinions: {len(expert)}")
    for e in expert:
        print(f"    {e['obs_id']}: {e['metric_name']} -- {e['metric_value'][:80]}...")

    # Technology lifecycle summary
    print(f"\n  Technology Lifecycle Summary:")
    for t in technologies:
        print(f"    {t['tech_name']}: {t['lifecycle_at_study']} (2004) -> {t['lifecycle_current']} (current)")
