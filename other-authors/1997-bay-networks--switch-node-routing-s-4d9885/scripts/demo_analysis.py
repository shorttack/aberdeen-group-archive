...
def main():
    ...
    print("")
    print("Predictions: " + str(len(preds)) + " | Verified outcomes: " + str(len(outcomes)))
    for pr in preds:
        print("  PRED [" + str(pr["year_observed"]) + "]: " + pr["metric_name"][:65])
    for ao in outcomes:
        print("  OUT  [" + str(ao["year_observed"]) + "]: " + ao["metric_name"][:65])
