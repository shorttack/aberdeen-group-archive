#!/usr/bin/env python3
"""Demo Analysis — Enterprise Executives Year 2000 Solution (1996)"""
import csv, os, sys
...
def main():
    print("="*60)
    print("DEMO ANALYSIS: Enterprise Executives & Y2K (1996)")
    print("="*60)
    ...
    for p in preds: print(f"  PRED: {p['metric_name']} -> {p['metric_value'][:60]}")
    for o in outs: print(f"  OUT:  {o['metric_name']} -> {o['metric_value'][:60]}")
