#!/usr/bin/env python3
"""Demo analysis script for aberdeen-1996-ibm-os2-warp-server dataset."""
import os, csv
...
def main():
    ...
    print("=" * 60)
    print("DATASET: aberdeen-1996-ibm-os2-warp-server")
    print("=" * 60)
    s = studies[0]
    ...
    if predictions:
        paired = sum(1 for p in predictions if any(
            o["entity_id"] == p["entity_id"] or o["tech_id"] == p["tech_id"]
            for o in outcomes))
        print(f"Paired predictions:    {paired}/{len(predictions)}")
    print()
