#!/usr/bin/env python3
"""Demo Analysis — IBM Transformation to Information-Service Solutions (1996)"""
import csv, os, sys
...
def main():
    print("="*60)
    print("DEMO ANALYSIS: IBM Transformation (1996)")
    print("="*60)
    ...
    for p in preds: print(f"  PRED: {p['metric_name'][:55]} -> {p['metric_value'][:40]}")
    for o in outs: print(f"  OUT:  {o['metric_name'][:55]} -> {o['metric_value'][:40]}")
    print("\n[KEY FINANCIAL METRICS (1995/1996)]")
    for o in observations:
        if o["observation_type"]=="market-data":
            print(f"  {o['metric_name'][:50]:<50} {o['metric_value'][:40]}")
