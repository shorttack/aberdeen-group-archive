...
Frictionless Data Package: ie-appendix-jones-wms-memo-2e5d6c

Availability of Client/Server WMS in 1994
Dr. Katherine Jones, Aberdeen Group ERP Research, June 7, 1999
...
def demo_analysis(root):
    ...
    for t in tech_assessments:
        print(f"    {t['obs_id']}: {t['metric_name']} = {t['metric_value'][:80]}")
    ...
    dissolved = [e for e in entities if e["status"] == "dissolved"]
    print(f"\n  Entities Dissolved: {len(dissolved)}")
    for d in dissolved:
        print(f"    {d['entity_name']} ({d['years_active']})")

    diminished = [e for e in entities if e["status"] == "diminished"]
    if diminished:
        print(f"\n  Entities Diminished: {len(diminished)}")
        for d in diminished:
            print(f"    {d['entity_name']} ({d['years_active']})")
