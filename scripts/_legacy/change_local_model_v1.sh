#!/usr/bin/env bash
# change_local_model_v1.sh — Install Qwen 3.6-27B (MLX) and verify before any script refactor.
#
...
# The registry tag-list endpoint is HTML; a simple HEAD on the manifest is the cheapest probe.
TAG_PAGE_URL="https://ollama.com/library/qwen3.6/tags"
if ! curl -fsS --max-time 10 "$TAG_PAGE_URL" | grep -q "qwen3.6:27b-mlx"; then
...
echo "  ok: $NEW_MODEL is present on the registry."
echo
