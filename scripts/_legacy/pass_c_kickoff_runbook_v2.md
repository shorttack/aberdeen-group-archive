...
**Document ID**: `pass_c_kickoff_runbook_v2`
**Date**: 2026-05-29
...
**Estimated wall clock**: ~12 hours (~2,562 obs × 16 s/obs)
...
| Pre-filter script | `pre_filter_scoreable_obs_v1.py` | `pre_filter_scoreable_obs_v3.py` |
| Scoreable input filename | `working/scoreable_obs_v1.csv` | `working/scoreable_obs_v3.csv` |
| Runner script | `run_prescience_pass_c_v1.py` | `run_prescience_pass_c_v2.py` |
...
```
cp scripts/pre_filter_scoreable_obs_v3.py \
   scripts/run_prescience_pass_c_v2.py \
...
## Step 1: Run the v3 pre-filter across Bucket A+B (dry-run first)

```
python3 /Users/scott/Desktop/Archive/scripts/pre_filter_scoreable_obs_v3.py \
  --root /Users/scott/Desktop/Archive/prepared \
  --dry-run
```

**Expected (within ~10 seconds)**: A table with one row per Bucket A+B study
showing total observations, scoreable, skipped. Bottom line `TOTAL` should be
roughly **~2,562 scoreable** across **~309 studies** (A=135 + B=174).
...
This writes `working/scoreable_obs_v3.csv`, `working/skipped_obs_v3.csv`, and
`working/filter_summary_v3.json` into each Bucket A+B study directory. It also
writes `prepared/_bucket_audit_v3.csv` summarizing the overall bucket
...
```
# only Bucket A
python3 .../pre_filter_scoreable_obs_v3.py --root ... --bucket-filter A
...
```
python3 /Users/scott/Desktop/Archive/scripts/run_prescience_pass_c_v2.py \
  --root /Users/scott/Desktop/Archive/prepared \
...
those with a `working/scoreable_obs_v3.csv` from Step 1) plus a total scoreable
count and an ETA in hours (should be ~11–13 hours). No Ollama calls, no
writes. Checkpoint file is also untouched.

If the dry-run says "0 studies with scoreable_obs_v3.csv", you skipped Step 1
...
```
nohup python3 /Users/scott/Desktop/Archive/scripts/run_prescience_pass_c_v2.py \
  --root /Users/scott/Desktop/Archive/prepared \
  --prompt /Users/scott/Desktop/Archive/prescience_score_prompt_v2.md \
