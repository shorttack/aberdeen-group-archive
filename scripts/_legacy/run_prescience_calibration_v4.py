#!/usr/bin/env python3
"""
run_prescience_calibration_v4.py
=================================
Three-way prescience-scoring calibration for v1.5 Bucket A Pass C.

v4 changes (§11q, 2026-06-02):
  - Defaults bumped from qwen3.5 -> qwen3.6 MLX line (both 27b and 35b).
    qwen3.6:27b-mlx pulled by change_local_model_v1.sh; qwen3.6:35b-mlx must
    be pulled separately via `ollama pull qwen3.6:35b-mlx` (~22 GB) before
    any 35b calibration run.
  - Old 3.5 models remain installed for 7-day rollback window (until 2026-06-09).
    To run against the old models, pass --models qwen3.5:27b-mlx,qwen3.5:35b-mlx.

v3 changes (perf):
  - think=False  (thinking-off mode; eliminates hidden think traces
    that were 95% of output tokens at 13 tok/sec on 3.5)
...
Calls:
  ...
  - Models: qwen3.6:27b-mlx, qwen3.6:35b-mlx (sequential, not parallel — 48 GB RAM)
...
Usage:
  python3 run_prescience_calibration_v4.py \\
...
def main():
    ...
    # §11q (2026-06-02): defaults bumped to Qwen 3.6 MLX line.
    # 27b is the daily-driver (pulled by change_local_model_v1.sh).
    # 35b must be pulled separately before any 35b calibration run.
    # Pass --models qwen3.5:27b-mlx,qwen3.5:35b-mlx to use the old models
    # during the 7-day rollback window (until 2026-06-09).
    ap.add_argument("--models", default="qwen3.6:27b-mlx,qwen3.6:35b-mlx",
