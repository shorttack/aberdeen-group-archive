#!/usr/bin/env python3
"""
01_load_csvs_v3.py — Phase 1: Load + validate master CSVs (v1.6)

v3 corrections vs v2 (2026-06-17 §11v cont 6):
  - Sentinel filter at ingest. Rows in _master_prescience_scores.csv where
    prescience_score < 0 are EXCLUDED from the obs join. They remain in the
    master CSV (auditable; source_pass column tells the story) but never
    reach the wiki parquets or DuckDB views. This is the single-chokepoint
    fix for the sentinel-leakage bug introduced when Tier B added 56 -1
    (prefilter_excluded) rows and SH added 16 -99 (content_unrecoverable)
    rows. Without this filter, prescience_mean would be dragged down and
    prescience_obs_count inflated for affected studies, plus per-obs page
    output would show literal '-1' / '-99' in markdown body text.
  - Sentinel taxonomy:
      -1  = parse_fail (SH) OR prefilter_excluded (Tier B); source_pass
            disambiguates (sh_sweep_v2 vs pass_c_prefilter_v1).
      -2  = pending (reserved; not emitted)
      -99 = content_unrecoverable (Sonar refusal, SH post-hoc reclass)
  - build_manifest.json now reports n_prescience_rows_filtered_as_sentinels.
  - Companion: Phase 2/3 require no changes — they read sentinel-clean parquet.
...
def main() -> int:
    ...
    # ---- Sentinel filter (v3, 2026-06-17 §11v cont 6) ----
    # Drop rows where prescience_score is a sentinel (< 0). These obs were
    # evaluated and rejected for known reasons (figure-caption prefilter,
    # parse-fail, content_unrecoverable refusal). Their existence is preserved
    # in the master CSV with source_pass disambiguating the reason; this Phase 1
    # filter prevents the sentinel values (-1, -2, -99) from corrupting
    # downstream aggregations (mean/max) and per-obs displays in the wiki.
    _presc_score_numeric = pd.to_numeric(presc["prescience_score"], errors="coerce")
    _sentinel_mask = _presc_score_numeric < 0
    n_presc_sentinels = int(_sentinel_mask.sum())
    n_presc_pre = len(presc)
    presc = presc[~_sentinel_mask].reset_index(drop=True)
    print(f"  sentinel filter — dropped {n_presc_sentinels} prescience rows "
          f"with score < 0; {len(presc)}/{n_presc_pre} retained")
    ...
    manifest["phase_1"] = {
        ...
        "derived": {
            ...
            "prescience_rows_filtered_as_sentinels": n_presc_sentinels,
