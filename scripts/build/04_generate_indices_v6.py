#!/usr/bin/env python3
"""
04_generate_indices_v4.py — Phase 4: Home, decades, collections, prescient (v1.6)

v4 changes (§11q, 2026-06-02):
  - PRESCIENT_INDEX `model=` template arg is now sourced from
    _llm_helper_v2.LOCAL_MODEL instead of hardcoded "qwen3.5:27b-mlx".
    The generated _prescient.md page now reflects whatever model is
    currently set as the local-scorer. Eliminates the Gotcha 9 drift
    where the doc string and the actual runtime model diverged.
  - No other behavior changes vs v3. Output schema unchanged.
  - Requires _llm_helper_v2.py in the same directory (build/) — same as v3
    in the canonical layout.

v3 corrections vs v2 (2026-06-01, WORKLIST §11l):
  - _prescient.md now shows aggregate totals for each section header
    ("Total: N studies ...") so `kw ask` retrieval surfaces the count
    not just the top-50 table. Totals are computed at build time from
    the studies parquet, never hardcoded.
...
# Helper lives next to this file in build/. If unimportable (script copied
# elsewhere), fall back to a hardcoded string that MUST match the helper.
try:
    _here = Path(__file__).resolve().parent
    sys.path.insert(0, str(_here))
    from _llm_helper_v4 import LOCAL_MODEL
except ImportError as _e:
    print(f"[indices v4] WARNING: could not import _llm_helper_v2 ({_e}); "
...
**Total: {total_obs} studies with prescience_max ≥ 4** (top 50 shown below; sorted by `prescience_max` then `prescience_mean`).
...
**Total: {total_holistic} studies with holistic rating `high`** (top 50 shown below; sorted by `pub_year`). Original-ingest holistic rating; complementary to the obs-level scores above.
...
def main() -> int:
    ...
    # Obs-level: every study with prescience_max >= 4 (full population, not just top 50)
    obs_level_all = studies[studies["prescience_max"].fillna(0) >= 4]
    total_obs = len(obs_level_all)
    top_obs = (obs_level_all
    ...
    # Holistic: study_prescience_enum == 'high' (full population, not just top 50)
    if "study_prescience_enum" in studies.columns:
        holistic_all = studies[studies["study_prescience_enum"].eq("high")]
    else:
        holistic_all = studies.iloc[0:0]
    total_holistic = len(holistic_all)
    holistic = (holistic_all
