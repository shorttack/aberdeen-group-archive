#!/usr/bin/env python3
"""
05_compute_embeddings_v4.py — Phase 5: Qwen3-Embedding-8B page embeddings (MRL-1024)

v4 (2026-07-01): default model bge-m3 -> qwen3-embedding:8b at 1024-dim via MRL
truncation on the OpenAI-compat /v1/embeddings endpoint. Schema unchanged from
v3 (6 cols). Vectors L2-normalized at embed time. kw_ask.py query side must use
the same model + dimensions. See embed() docstring below for the full rationale.
...
def _l2_normalize(vec: list[float]) -> list[float]:
    """L2-normalize. MRL truncation (4096->1024) breaks unit norm; Qwen3-Embedding
    is a cosine model so stored vectors must be renormalized."""
    import math
    n = math.sqrt(sum(x * x for x in vec)) or 1.0
    return [x / n for x in vec]


def embed(text: str, model: str, dimensions: int = 1024) -> list[float] | None:
    """Embed via Ollama's OpenAI-compatible /v1/embeddings endpoint.

    Unlike native /api/embeddings, /v1/embeddings accepts a `dimensions` param,
    triggering server-side MRL truncation for Matryoshka-trained models
    (qwen3-embedding). Pass dimensions=0 to omit the param (native dim, bge-m3).
    """
    ...
    try:
        ...
        if not data:
            print("  [warn] embed returned no data")
            return None
...
def main() -> int:
    ...
    ap.add_argument("--dimensions", type=int, default=1024,
                    help="MRL truncation dim via /v1/embeddings (0 = native, bge-m3)")
    ap.add_argument("--out", default="data/embeddings.parquet",
                    help="Output parquet path relative to --wiki (side path for A/B)")
    ...
    dim_note = f"@{args.dimensions}d (MRL)" if args.dimensions > 0 else "@native-dim"
    print(f"Embedding {len(pages)} pages using {args.model} {dim_note}...")
    ...
    for i, p in enumerate(pages, 1):
        vec = embed(p["embed_input"], args.model, args.dimensions)
        rows.append({
