#!/usr/bin/env python3
"""
02_build_data_layer_v4.py — Phase 2: Parquet → DuckDB (v1.5)

v4 corrections vs v3 (WORKLIST §4c follow-up — 2026-05-31):
  - v3 claimed to fix the decade-bucket bug but did not. v3's expression
    `(CAST(pub_year AS INTEGER) / 10) * 10 || 's'` produced `'1972.0s'`
    strings and 38 distinct buckets (one per year) instead of 6 (one per
    decade). Root cause: DuckDB's `/` operator on INTEGER returns DOUBLE
    (verified by `typeof(CAST(1972.0 AS INTEGER) / 10) -> DOUBLE`). So
    `CAST(1972.0 AS INTEGER) / 10` = `197.2` (double), `* 10` = `1972.0`
    (double), `|| 's'` = `'1972.0s'`.
  - v4 uses DuckDB's integer-division operator `//`:
    `(CAST(pub_year AS INTEGER) // 10) * 10 || 's'`
    `CAST(1972.0 AS INTEGER) // 10` = `197` (integer), `* 10` = `1970`
    (integer), `|| 's'` = `'1970s'`. Verified empirically against
    `~/Repos/kastner-aberdeen-wiki/db/kastner.duckdb` on 2026-05-31
    before shipping.
  - No other changes vs v3. Same view set, same arguments, same outputs.

v3 corrections vs v2 (WORKLIST §4c — 2026-05-29) [HISTORICAL — INEFFECTIVE]:
  - Fix v_studies_by_decade + v_prescience_by_decade: previous expression
    `CAST(pub_year / 10 * 10 AS INTEGER)` returned per-year rows because
    pub_year is stored as DOUBLE in the parquet (pandas float64 when nulls
    present). DOUBLE math kept the fractional precision and CAST only
    truncated at the end, yielding 1997.0 → 1997 → '1997s' instead of
    '1990s'. Proposed expression `(CAST(pub_year AS INTEGER) / 10) * 10 || 's'`
    — author's intent was to cast first so `/` and `*` would use integer
    arithmetic, but DuckDB returns DOUBLE from `/` on INTEGER, so the fix
    did not take. See v4 above for the real fix.
...
Usage:
  python3 02_build_data_layer_v4.py --wiki ~/Repos/kastner-aberdeen-wiki
"""
