#!/usr/bin/env python3
"""
06_emit_scaffolding_v5.py — Phase 6: README, AGENTS, chat-starter, Makefile,
verify + semantic_search scripts (v1.5).

Usage:
  python3 06_emit_scaffolding_v5.py --wiki ~/Repos/kastner-aberdeen-wiki
"""
...
# at write time with the current value of LOCAL_MODEL from _llm_helper_v4.
...
#
# v5 (2026-06-18): fallback model corrected to `qwen3.5:27b-mlx` (the active
# model after qwen3.6 was REMOVED, per local-model-upgrade-gates skill / EOD
# 2026-06-17). v4 had stale `qwen3.6:27b-mlx` fallback that would mis-label
# the README/AGENTS if the helper import failed.
try:
    _here = Path(__file__).resolve().parent
    sys.path.insert(0, str(_here))
    from _llm_helper_v4 import LOCAL_MODEL
except ImportError as _e:
    print(f"[scaffolding v5] WARNING: could not import _llm_helper_v4 ({_e}); "
...
phase6:
\tpython3 scripts/build/06_emit_scaffolding_v5.py --wiki .
...
def main() -> int:
    ...
    files = {
        ...
        "Makefile": MAKEFILE,
        ".gitignore": (
            "# Generated by 06_emit_scaffolding_v5.py — hand-edits to this file\n"
            "# will be overwritten on next Phase 6 run. Add new patterns to the\n"
            "# GITIGNORE template inside the script, not here.\n"
            "\n"
            "# Working files / OS / editor\n"
            "*.tmp\n"
            ".obsidian/workspace*\n"
            ".DS_Store\n"
            "\n"
            "# Python (added v5 2026-06-18 — was missing in v1-v4, hand-patched\n"
            "# at commit 9df154f4 and immediately clobbered by Phase 6 v1 on rerun)\n"
            "__pycache__/\n"
            "*.pyc\n"
            "*.pyo\n"
            "\n"
            "# Backup files (sed-patch artifacts, regen .bak_* etc.; same regression)\n"
            "*.bak_*\n"
        ),
