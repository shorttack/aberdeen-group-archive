#!/usr/bin/env python3
"""
build_format_mismatch_review_v2.py
...
After v4 partitioned the 437 `pdf_format_mismatch` rows into:
  ...
  F6_detector_saw_different_title     =   1  (Pete-review — two real headlines, one wins)
  F5_normalization_rule_gap           =   6  (Pete-review — detector & CSV both reasonable)
...
v2 change vs v1: F5 added to SALVAGE_BUCKETS. v1 ran on Mac 2026-06-19 13:11 UTC
and emitted only 21 rows; v4 diagnostic counted these 6 F5-like rows under F6
because v4 didn't expose the F5 branch. v2 surfaces all 27 ambiguous rows for
Pete-as-arbiter review.
...
OUT_CSV = QUOTATIONS_DIR / "_format_mismatch_review_v2.csv"
...
CONFIDENCE = {
    ...
    "F5":  "medium",   # detector & CSV both reasonable; normalization-rule gap (v2)
    "F6":  "low",      # two real-looking headlines disagree — Pete picks one
}

# Salvageable buckets we emit to the review CSV
# v2: F5 added so Pete can arbitrate normalization-rule ambiguity per-row.
SALVAGE_BUCKETS = {"F0a", "F1", "F3", "F5", "F6"}


def main(commit: bool = False):
    print(f"[build_format_mismatch_review_v2] {datetime.now(timezone.utc).isoformat()}")
