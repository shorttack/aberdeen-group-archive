#!/usr/bin/env bash
# change_local_model_v3.sh — Install Qwen 3.6-27B (MLX) and verify before any script refactor.
#
...
if [ "$COMMIT" = "1" ]; then
  ...
  # v3 fix: `think` MUST be top-level for Ollama 0.24+. v2 nested it under
  # `options` (ignored), the thinking model swallowed all 4 tokens of
  # num_predict inside <think>, and `.response` came back empty even though
  # the model was healthy. Top-level + a bit more num_predict headroom.
  SMOKE_RESPONSE=$(curl -fsS --max-time 60 "$OLLAMA_BASE/api/generate" \
    -H "Content-Type: application/json" \
    -d "{\"model\":\"$NEW_MODEL\",\"prompt\":\"Reply with the single word OK and nothing else.\",\"stream\":false,\"think\":false,\"options\":{\"num_predict\":16,\"temperature\":0}}" \
