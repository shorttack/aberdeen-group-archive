#!/usr/bin/env python3
"""
detect_article_boundaries_v1.py
================================
...
Mac:
  cd ~/Desktop/Archive
  python3 ~/Desktop/Archive/scripts/detect_article_boundaries_v1.py
  # → DRY-RUN diagnostics printed to stdout
  python3 ~/Desktop/Archive/scripts/detect_article_boundaries_v1.py --commit
...
def match_articles_to_csv(
    ...
    # Build searchable detected-article corpus
    detected_headlines_norm = [
        normalize_text(a["headline"] or "") for a in articles
    ]
    detected_set = set(detected_headlines_norm)
    ...
    return (matched, len(csv_articles), unmatched)
...
def run(commit: bool = False) -> None:
    print(f"[detect_article_boundaries_v1] {datetime.now(timezone.utc).isoformat()}")
    ...
    matched, total_csv, unmatched = match_articles_to_csv(articles, CSV_PATH)
