"""
diag_admit_orphan_sources_v2.py — investigate where the 10 admit-orphan
...
def main() -> int:
    print("=== diag_admit_orphan_sources_v2 ===\n")
    ...
    if UNCLAIMED.exists():
        ...
        for s in segs:
            if isinstance(s, dict):
                # Real schema uses raw_preview (truncated body) + headline_attempted
                t = (s.get("raw_preview") or s.get("text") or s.get("body") or "") + " " + \
                    (s.get("headline_attempted") or "") + " " + (s.get("reason") or "")
            else:
                t = str(s)
    ...
    for rid, h, nh in orphans:
        ...
        # Classification (P1 = in corpus body of another article; P2 = in unclaimed PDF;
        # P3 = in Media.md; P4 = in raw RTF text).
        # Class precedence: A (recoverable from unclaimed PDF) > E (cited in another
        # corpus article — accept as cross-reference) > C (RTF has it, extractor gap)
        # > B (pending ingest) > D (nowhere).
        if p2:                  cls = "A_recoverable_PDF_segment"
        elif p1:                cls = "E_cited_in_corpus_body"
        elif p4:                cls = "C_RTF_extractor_gap"
        elif p3:                cls = "B_pending_media_md"
        else:                   cls = "D_external_only"

        triage_counts[cls] = triage_counts.get(cls, 0) + 1
