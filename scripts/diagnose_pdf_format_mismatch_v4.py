#!/usr/bin/env python3
"""
diagnose_pdf_format_mismatch_v4.py
...
def main():
    ...
    for row in mismatch_rows:
        ...
        if len(probe_norm) < 20:
            # v4: Probe-from-quote failed (too short). Fall back to probe-from-
            # headline. If headline appears in any PDF segment body, the article
            # IS in the scan — the union normalizer just rejected the join.
            # → F0a (substrate-salvageable). Otherwise → F0b (roll up to F4).
            h_norm = _norm(headline)
            if len(h_norm) < 12:
                failure_mode["F0c_both_probes_too_short"] += 1
                if len(samples["F0c_both_probes_too_short"]) < 3:
                    samples["F0c_both_probes_too_short"].append({
                        "row_id": row.get("row_id", ""),
                        "headline": headline[:100],
                        "quote_first_100": quote[:100],
                        "note": f"q_len={len(probe_norm)} h_norm_len={len(h_norm)}",
                    })
                continue
            ...
            if headline_found_in:
                ...
                if kind == "corpus":
                    failure_mode["F0a_headline_in_corpus_norm_gap"] += 1
                    if len(samples["F0a_headline_in_corpus_norm_gap"]) < 3:
                        samples["F0a_headline_in_corpus_norm_gap"].append({
                            "row_id": row.get("row_id", ""),
                            "csv_headline": headline[:100],
                            "corpus_article_seq": sid,
                            "corpus_article_headline": sdata["headline"][:100],
                            "quote_first_100": quote[:100],
                        })
                else:
                    failure_mode["F0a_headline_in_unclaimed_norm_gap"] += 1
                    if len(samples["F0a_headline_in_unclaimed_norm_gap"]) < 3:
                        samples["F0a_headline_in_unclaimed_norm_gap"].append({
                            "row_id": row.get("row_id", ""),
                            "csv_headline": headline[:100],
                            "seg_idx": sid,
                            "seg_class": sdata.get("classification", ""),
                            "detector_saw": (sdata.get("headline_attempted") or "")[:100],
                        })
            else:
                failure_mode["F0b_headline_also_not_in_pdf"] += 1
                if len(samples["F0b_headline_also_not_in_pdf"]) < 3:
                    samples["F0b_headline_also_not_in_pdf"].append({
                        "row_id": row.get("row_id", ""),
                        "headline": headline[:100],
                        "quote_first_100": quote[:100],
                    })
    ...
    salvageable = {
        ...
        "F0a_headline_in_corpus_norm_gap": "salvageable (loosen union normalizer)",
        "F0a_headline_in_unclaimed_norm_gap": "salvageable (fuzzy-match headline)",
        "F0b_headline_also_not_in_pdf": "NOT salvageable (article not in PDF scan)",
        "F0c_both_probes_too_short": "NOT diagnosable (both probes too short)",
    }
    print(f"=== Salvageability ===")
    sal_modes = ["F1_detector_grabbed_midsentence",
                 "F2_headline_in_adjacent_segment",
                 "F3_headline_in_body_detector_missed",
                 "F5_in_corpus_join_failed",
                 "F0a_headline_in_corpus_norm_gap",
                 "F0a_headline_in_unclaimed_norm_gap"]
    sal_count = sum(failure_mode.get(m, 0) for m in sal_modes)
    not_sal = (failure_mode.get("F4_not_in_pdf", 0)
               + failure_mode.get("F0b_headline_also_not_in_pdf", 0))
    review = failure_mode.get("F6_detector_saw_different_title", 0)
    untestable = failure_mode.get("F0c_both_probes_too_short", 0)
    print(f"  substrate-salvageable             : {sal_count} / {total} ({100.0*sal_count/total:.1f}%)")
    print(f"  Pete-review (different title)     : {review}")
    print(f"  NOT salvageable (not in PDF)      : {not_sal}")
    print(f"  NOT diagnosable (probes too short): {untestable}")
    print()
