#!/usr/bin/env bash
# overnight_v2.sh — 2026-07-07 evening cleanse + full SH-chain rebuild.
#
# Sequence:
...
for p in "$REPO/_master_studies.csv" \
         ...
         "$REPO/tech_mislabel_candidates_v1.csv" \
         "$REPO/entity_metadata_candidates_v1.csv" \
         "$REPO/entity_alias_map_v1_sap_only.csv" \
...
run_phase "phase_A_tech_mislabel_DRYRUN"    python3 "$SCRIPTS/apply_tech_mislabel_v1.py" \
                                              --candidates "$REPO/tech_mislabel_candidates_v1.csv"
run_phase "phase_A_tech_mislabel_COMMIT"    python3 "$SCRIPTS/apply_tech_mislabel_v1.py" \
                                              --candidates "$REPO/tech_mislabel_candidates_v1.csv" --commit

run_phase "phase_B_entity_metadata_DRYRUN"  python3 "$SCRIPTS/apply_entity_metadata_v1.py" \
                                              --candidates "$REPO/entity_metadata_candidates_v1.csv"
run_phase "phase_B_entity_metadata_COMMIT"  python3 "$SCRIPTS/apply_entity_metadata_v1.py" \
                                              --candidates "$REPO/entity_metadata_candidates_v1.csv" --commit

run_phase "phase_C_sap_alias_DRYRUN"        python3 "$SCRIPTS/apply_entity_aliases_v1_sap.py" \
                                              --candidates "$REPO/entity_alias_map_v1_sap_only.csv"
run_phase "phase_C_sap_alias_COMMIT"        python3 "$SCRIPTS/apply_entity_aliases_v1_sap.py" \
                                              --candidates "$REPO/entity_alias_map_v1_sap_only.csv" --commit
