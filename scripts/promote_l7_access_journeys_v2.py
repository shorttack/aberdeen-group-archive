#!/usr/bin/env python3
"""
promote_l7_access_journeys_v2.py

RUN THIS ON THE MAC DIRECTLY (a Terminal shell with full filesystem perms) —
NOT through the pc bash sandbox, which cannot write to ~/Desktop/Archive/.
Dry-run is default and safe to run anywhere; --commit must run on the Mac.

v2 (2026-06-28): corrected junction-link expectations after v1 dry-run.
  v1 assumed +6 entity / +8 tech links (16/9 access links). Reality: the
  journeys reference 9 PCmaker entity_ids (already all linked, legacy=10 incl
  best-buy) and only 4 CPU tech_ids actually used as obs tech_id. So entity
  links +0 (stay 10) and tech links +4 (1 -> 5). Retailers are channel text
  inside journey values, not obs entity_id; the 5 unused package techs are not
  linked (would be phantom study links with no supporting observation).
...
EXPECTED DELTAS (verified by v1 dry-run 2026-06-28; --commit asserts them):
  observations   : -122 legacy +249 journeys  (net +127 for this study)
  entity_studies : +0   (legacy 10 PCmaker+best-buy links already cover the 9
                         journey PCmakers; retailers appear as channel text
                         inside journey values, not as obs entity_id)
  tech_studies   : +4   (1 -> 5: desktop-pc kept; + the 4 CPU classes the
                         journeys actually reference as tech_id:
                         amd-athlon, intel-celeron, intel-pentium-4,
                         intel-pentium-4-ht)
  studies        : 0 row delta (1 row updated in place)

Note: only tech_ids ACTUALLY referenced by a journey obs get a junction link.
The staged technologies.csv lists 9 techs, but only 4 are used as obs tech_id;
linking the unused 5 (ddr-sdram, ddr-sdram, windows-xp, etc.) would create
phantom study links with no supporting observation, so we do not.
"""
...
print(f"adding {len(add_ej)} entity links (expected 0): {[r[0] for r in add_ej]}")
...
print(f"adding {len(add_tj)} tech links (expected 4): {[r[0] for r in add_tj]}")
...
# --- backups (in-place; requires Mac filesystem perms, NOT the pc sandbox) ---
banner("BACKUPS")
...
acc_e = sum(1 for r in rb if r[1] == CANON_SID)
print(f"entity_studies: {len(rb)} (expect {len(final_ej)}); access links {acc_e} (expect 10)")
assert acc_e == 10 and len(rb) == len(final_ej)

h, rb = read_csv(M_TECHJ)
assert h == ["tech_id", "study_id"]
acc_t = sum(1 for r in rb if r[1] == CANON_SID)
print(f"tech_studies: {len(rb)} (expect {len(final_tj)}); access links {acc_t} (expect 5)")
assert acc_t == 5 and len(rb) == len(final_tj)
...
print("NEXT (on Mac): Phase 1 + Phase 2 rebuild, then Phases 3-6 "
