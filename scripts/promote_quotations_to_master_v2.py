...
low_signal_flag rule (v2 — 2026-06-19 §11x revision after v1 dry-run analysis):
...
def compute_low_signal_flag(quote: str) -> bool:
    """Return True if quote contains a known blog-scrape platform artifact.

    Rule (v2, 2026-06-19 §11x):
      flag = TRUE if quote (case-insensitive) contains any pattern in
      BLOG_ARTIFACT_PATTERNS.

    Word-count and possessive-name rules from v1 were dropped: empirical
    inspection showed the actual degenerate shape is long-form web-scrape
    artifact text, not short attribution lines.
    """
