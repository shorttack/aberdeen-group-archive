#!/usr/bin/env python3
"""
reconcile_masters_mac_to_repo_v1.py
====================================
...
Usage:
    ...
    python3 reconcile_masters_mac_to_repo_v1.py
    # 2. Inspect output, confirm pre-flight passes.
    # 3. Commit:
    python3 reconcile_masters_mac_to_repo_v1.py --commit
...
WORKSPACE_ARTIFACTS = {
    ...
    "scripts/reconcile_masters_mac_to_repo_v1.py": Path(__file__).resolve(),
}
...
def gh_create_blob_from_file(path: Path, label: str) -> str:
    ...
    tmp = Path(f"/tmp/_blob_req_{label}.json")
...
def resolve_workspace_artifact(repo_path: str, ws_path: Path) -> Path:
    ...
    candidate = MAC_STAGING_DIR / Path(repo_path).name
    if candidate.is_file():
        return candidate
    # Try Mac-staged copy under full repo path
    candidate2 = MAC_STAGING_DIR / repo_path
...
def main() -> int:
    ...
    backup_dir = f"archive_masters_pre_reconcile_{timestamp}Z"
    ...
    backup_readme_bytes = build_backup_readme(timestamp + "Z", facts["repo_blobs"],
                                              facts["main_sha"])
    plan_rows.append({
        "op": "CREATE",
    ...
    plan_rows.append({
        "op": "CREATE",
        "repo_path": "scripts/reconcile_masters_mac_to_repo_v1.py",
    ...
    # 6. This script
    self_sha = gh_create_blob_from_file(self_path, "scripts/reconcile_masters_mac_to_repo_v1.py")
    tree_entries.append({
        "path": "scripts/reconcile_masters_mac_to_repo_v1.py",
