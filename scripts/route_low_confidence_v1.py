...
============================
Async cloud reviewer for Pass C confidence==1 rows.

Runs AFTER Pass C completes. Per the Model Prescience Scoring Finding (decision §5
...
that gets dropped into each study's working/prescience_scores_cloud_review_v1.csv.

Workflow:
  1. Sweep prepared/<study>/working/prescience_scores_27b_passC_v1.csv files
  2. Filter rows where confidence == 1 (or 0 — total non-claim per cloud)
  3. Emit pass_c_cloud_review_queue_v1.csv at --root with the obs payload
  ...
  4. Computer then runs the cloud scoring in-session, writes results to
     <root>/pass_c_cloud_review_results_v1.csv with the standard 11-column schema
  5. apply_cloud_review_v1.py (separate step, included below as --apply) splits
     the results CSV back into per-study prescience_scores_cloud_review_v1.csv
     files so the rollup picks them up.

Usage:
  # build the queue
  python3 route_low_confidence_v1.py \\
      --root /Users/scott/Desktop/Archive/prepared \\
      --build-queue

  # (agent fills pass_c_cloud_review_results_v1.csv)
  ...
  python3 route_low_confidence_v1.py --root ... --build-queue --dry-run
"""
...
PASS_C_GLOB = "working/prescience_scores_27b_passC_v1.csv"
QUEUE_FILENAME = "pass_c_cloud_review_queue_v1.csv"
RESULTS_FILENAME = "pass_c_cloud_review_results_v1.csv"
...
def build_queue(root: Path, dry_run: bool) -> int:
    ...
    print(f"Studies with Pass C output: {n_studies_seen}")
    ...
    print(f"Then run: python3 route_low_confidence_v1.py --root {root} --apply")
    return 0
...
def apply_results(root: Path) -> int:
    ...
    for sid, sid_rows in by_study.items():
        ...
        out = working / "prescience_scores_cloud_review_v1.csv"
    ...
    print("Next step: rerun roll_up_prescience_to_master_v1.py to merge.")
    return 0
...
def main() -> int:
    ...
    mode.add_argument("--build-queue", action="store_true",
                      help="Sweep Pass C output, emit cloud review queue")
