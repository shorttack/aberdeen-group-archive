#!/usr/bin/env python3
"""
run_prescience_calibration_v7_qwen_30obs.py
...
Writes:
    Perplexity_Only/calibration_v7_qwen_scores.csv          (12-col v2 schema)
    Perplexity_Only/calibration_v7_qwen_spool.jsonl         (append-only)
    Perplexity_Only/calibration_report_v7.md                (kappas + go/no-go)
...
OUT_SCORES   = PERPLEXITY_ONLY / "calibration_v7_qwen_scores.csv"
OUT_SPOOL    = PERPLEXITY_ONLY / "calibration_v7_qwen_spool.jsonl"
OUT_REPORT   = PERPLEXITY_ONLY / "calibration_report_v7.md"

MODEL_NAME    = "qwen3.5:27b-mlx"   # adjust if Ollama tag differs
SCORER_VERSION = "pass_c_v2_calib_v7"   # v7: align Qwen rubric to canonical 1-5 scale used by Sonar+Claude
...
PROMPT_TEMPLATE = """You are a prescience scorer for Aberdeen Group technology industry observations.

Given a claim made at a known point in time, rate how prescient the claim turned
out to be in light of what actually happened in the subsequent years.
...
Score on the canonical Aberdeen 1-5 prescience scale:
  1 = Wrong / contradicted by subsequent events
  2 = Partially wrong; missed key dynamics or right for wrong reasons
  3 = Mixed / ordinary forecasting; directionally plausible but not insightful
  4 = Substantially correct with reasonable lead time and above-average insight
  5 = Strikingly correct with strong lead time and non-obvious framing

Confidence is also on a 1-5 integer scale:
  1 = very low confidence in your score (sparse evidence, ambiguous outcome)
  5 = very high confidence (strong evidence, unambiguous outcome)

Respond with ONLY a JSON object on a single line, no prose, no markdown:
{{"score": <int 1-5>, "confidence": <int 1-5>, "rationale": "<<= 240 chars>"}}
"""
...
def parse_score(raw: str):
    """Return (score:int|None, confidence:int|None, rationale:str, parse_ok:bool).
    v7: both score and confidence are integers on the 1-5 Aberdeen scale.
    """
    raw = (raw or "").strip()
    if raw.startswith("```"):
    ...
    try:
        ...
        s = int(obj.get("score"))
        # confidence: accept int 1-5 OR (defensive) float in 0-1 which we map up
        c_raw = obj.get("confidence", 0)
        if isinstance(c_raw, float) and 0.0 <= c_raw <= 1.0:
            c = max(1, min(5, round(c_raw * 4) + 1))   # map 0-1 -> 1-5
        else:
            c = int(c_raw)
...
# ----- KAPPA (quadratic-weighted, 1-5 scale) -----
def bin_score(s: int) -> int:
    """v7: 1-5 Aberdeen scale -> 0..4 bin index. Pre-filter (<=0) is invalid here
    and should be filtered before reaching bin_score."""
...
def main():
    ...
    print(f"[v7] manifest={len(manifest)}  obs_master={len(obs_lut)}  existing_scored_obs={len(existing)}")
        ...
        with open(OUT_SPOOL, encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    pass
        print(f"[v7] resume: {len(done)} already scored in spool")
    ...
    print(f"[v7] wrote {OUT_SCORES} ({len(all_rows)} rows)")
    ...
    report = f"""# Pass C v2 Calibration Report (v7 / Qwen / 30 obs / 1-5 scale)
...
- Scale: canonical Aberdeen 1-5 (matches Sonar+Claude existing rows)
...
{"Both calibration kappas meet the >= " + str(KAPPA_GATE) + " gate. Proceed to write `run_prescience_pass_c_v7_qwen_full.py`." if go else "At least one kappa below gate. Inspect disagreements before scaling. Review the per-obs scores in `calibration_v7_qwen_scores.csv` and the rationales for the largest bin-distance pairs."}

## Notes
- Batch 3 (10 transcript obs) had no prior scores; Qwen scores are recorded in `calibration_v7_qwen_scores.csv` for spot-checking but contribute no kappa.
- Binning: 1 / 2 / 3 / 4 / 5 (canonical Aberdeen 1-5 scale).
"""
    OUT_REPORT.write_text(report, encoding="utf-8")
    print(f"[v7] wrote {OUT_REPORT}")
    print(f"[v7] kappas: B1={fmt(k_b1)}  B2={fmt(k_b2)}  ref={fmt(k_ov)}  decision={'GO' if go else 'NO-GO'}")
