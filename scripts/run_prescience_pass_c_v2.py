...
v2 differences vs v1 (2026-05-29):
  - Reads working/scoreable_obs_v3.csv (pre_filter_scoreable_obs_v3.py output)
...
Pipeline per study:
  1. Read working/scoreable_obs_v3.csv (REQUIRED — fail if missing)
...
SCOREABLE_FILENAME = "scoreable_obs_v3.csv"
...
def score_study(study_dir: Path, model: str, system_prompt: str,
    ...
    if not scoreable:
        print(f"  [skip] {study_id}: no scoreable_obs_v3.csv "
              f"(run pre_filter_scoreable_obs_v3.py first)")
        return 0, 0
...
def list_studies_with_scoreable(root: Path) -> list[Path]:
    """Studies that have a scoreable_obs_v3.csv from the pre-filter."""
...
def main() -> int:
    ...
    print(f"Studies with scoreable_obs_v3.csv: {len(studies)}")
