...
SCORER_VERSION = "v7"  # scoring methodology/prompt unchanged from v7; the v8 file only adds
                       # timeout-resilient retries + --study-ids scoping. Keeping the column
                       # value "v7" is honest provenance: identical score semantics.
SOURCE_PASS_API = "pass_c_sonar_v1"
SOURCE_PASS_PREFILTER = "pass_c_prefilter_v1"

REQUEST_TIMEOUT = 120  # v8: raised 60->120; sonar-reasoning-pro is slow, long runs hit read timeouts
SLEEP_BETWEEN = 0.3
MAX_RETRIES = 5     # v8: raised 3->5 for unattended long sweeps
RETRY_BACKOFF = 3.0
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            return {
                ...
                KeyError, ValueError, TimeoutError, socket.timeout, ConnectionError,
                OSError) as e:
            # v8 CRITICAL FIX: TimeoutError (bare, from socket read timeout on Py3.14)
            # was NOT in this tuple in v7, so a single API read-timeout propagated up
            # and killed the entire run. A ~4500-call sweep is near-certain to hit
            # several timeouts; catching them here lets the retry loop handle them.
            last_err = e
...
def main():
    ...
    ap.add_argument("--study-ids", type=str, default=None,
                    help="v8: comma-separated study_ids to scope scoring to "
                         "(first-class replacement for the --input-manifest workaround). "
                         "Only unscored obs in these studies are scored.")
    args = ap.parse_args()
    ...
    # v8: optional first-class study-id scoping (replaces the --input-manifest workaround)
    if args.study_ids:
        want = {s.strip() for s in args.study_ids.split(",") if s.strip()}
        before = len(rows)
        rows = [r for r in rows if r["study_id"] in want]
        print(f"[scope] --study-ids filter: {len(rows)} rows in {len(want)} studies (from {before})")
