...
SP_PREFILTER = "pass_c_sh_prefilter_thin"   # v9: routed to -1, no API call
...
# v9: thinness detection + richer obs_text -----------------------------------
# Root cause fixed in v9: v8 built obs_text from metric_value ALONE, so bare
# values like "79%" or "[UNVERIFIED]" gave the model nothing to reason over and
# it confabulated context (scored 4-5 on fragments). v9 composes the full claim
# from metric_name + metric_value + notes, matching what the fragment audit
# (2026-06-29) showed is actually scorable: ~98% of rows carry real context once
# name+notes are included. The truly-thin ~1.7% (no context in any field) are
# routed to -1 via --prefilter-thin instead of being sent to the API.

import re as _re_v9

_BARE_NUM_RE = _re_v9.compile(r"^[\s$\u20ac\u00a3%0-9.,:/+\-\u2014\u2013<>~()xX]*$")
THIN_RESCUE_WORDS = 5   # metric_name + notes word count needed to "rescue" a thin value
...
def compose_obs_text(row: dict) -> str:
    """Build the observation text the model reasons over from ALL claim fields.

    v8 used metric_value alone -> confabulation on bare fragments. v9 assembles
    metric_name + metric_value + notes so the scorer sees the real claim.
    """
    mn = (row.get("metric_name") or "").strip()
    mv = (row.get("metric_value") or "").strip()
    notes = (row.get("notes") or "").strip()
    parts = []
    if mn:
        parts.append(f"Metric: {mn}")
    if mv:
        parts.append(f"Value: {mv}")
    if notes:
        parts.append(f"Notes: {notes}")
    return "\n".join(parts) if parts else (mv or "")


def is_thin_obs(row: dict) -> bool:
    """True only when the row carries no scorable claim in ANY field.

    Matches the fragment-audit Pass-2 logic (2026-06-29): a thin metric_value is
    "rescued" if metric_name + notes carry >= THIN_RESCUE_WORDS words. Fully empty
    rows and bare-fragment rows with no rescuing context are truly thin (-> -1).
    """
    mv = (row.get("metric_value") or "").strip()
    mn = (row.get("metric_name") or "").strip()
    notes = (row.get("notes") or "").strip()

    mv_thin = (len(mv) == 0) or bool(_BARE_NUM_RE.match(mv)) or (_wc(mv) <= 3 and len(mv) <= 25)
    if not mv_thin:
        return False
    # thin value: rescued only if name+notes carry real claim text
    return (_wc(mn) + _wc(notes)) < THIN_RESCUE_WORDS
...
def row_prefilter_thin(obs_id: str, study_id: str, anchor_year: int,
                       anchor_source: str) -> dict:
    """v9: truly-thin obs (no scorable claim in any field) -> -1, no API call."""
    msg = "prefilter:thin_obs_no_scorable_claim"
    row = _empty_sh_row(obs_id, study_id)
    row.update({
        "prescience_3y": "-1", "confidence_3y": "", "rationale_3y": msg,
        "prescience_5y": "-1", "confidence_5y": "", "rationale_5y": msg,
...
def main():
    ...
    ap.add_argument("--workers", type=int, default=1,
                    help="Concurrent API workers (v10). 1=serial (v9 behavior). "
                         "8-12 recommended for sonar-pro; back off if 429s appear.")
    ...
    ap.add_argument("--prefilter-thin", action="store_true",
                    help="Route truly-thin obs (no scorable claim in any field) "
                         "to prescience -1 without an API call (v9; fragment audit 2026-06-29)")
    args = ap.parse_args()
    ...
    for r in rows:
        ...
        # v9: route truly-thin obs to -1 before anything else (no API call).
        # Try to resolve anchor for provenance, but thinness wins regardless.
        if args.prefilter_thin and is_thin_obs(r):
            try:
                _a = resolve_anchor_year(r, study_row)
                path_prefilter.append((r, _a.year, _a.source))
            except AnchorResolutionError:
                path_prefilter.append((r, 0, ""))
            continue
        try:
    ...
    print(f"[classify] prefilter_thin (no API):  {len(path_prefilter)}")
    ...
    for r, ay, asrc in path_prefilter:
        append_csv_row(output_csv,
                       row_prefilter_thin(r["obs_id"], r["study_id"], ay, asrc))
    print(f"[prefilter_thin] wrote {len(path_prefilter)}")
    ...
    print(f"[api] starting {len(api_queue)} calls with {workers} worker(s)...")
    t_start = time.monotonic()

    # Shared mutable state guarded by locks. File appends are serialized so the
    # output CSV / jsonl logs never interleave a half-written row across threads.
    write_lock = threading.Lock()
    stat_lock = threading.Lock()
    counters = {"ok": 0, "fail": 0, "done": 0}

    def score_one(item):
        """Thread worker: one observation -> one output row. Pure per-obs work;
        call_sonar is self-contained (no shared state). Only file writes + the
        progress counters touch shared state, both under locks."""
        mode, r, anchor = item
        obs_id = r["obs_id"]
        study_id = r["study_id"]
        study_row = studies.get(study_id, {})
        ...
        if err or not parsed:
            ...
            fail_obj = {"obs_id": obs_id, "mode": mode, "err": err, "raw_head": raw[:400]}
            chk_obj = None
        else:
            ok, norm = (validate_combined(parsed) if combined
                        else validate_3y_only(parsed))
            if not ok:
                out = row_parse_fail(obs_id, study_id, anchor.year, anchor.source,
                                     SP_COMBINED if combined else SP_3Y_ONLY, raw,
                                     "schema_mismatch")
                fail_obj = {"obs_id": obs_id, "mode": mode,
                            "err": "schema_mismatch", "raw_head": raw[:400]}
                chk_obj = None
            else:
                out = (row_combined_ok(obs_id, study_id, anchor.year, anchor.source, norm, raw)
                       if combined else
                       row_3y_only_ok(obs_id, study_id, anchor.year, anchor.source, norm, raw))
                is_ok = True
                fail_obj = None
                chk_obj = {"obs_id": obs_id, "mode": mode,
        ...
        with write_lock:
            ...
            if fail_obj is not None:
                append_jsonl(failure_log, fail_obj)
            if chk_obj is not None:
                append_jsonl(checkpoint, chk_obj)
        ...
        with stat_lock:
            ...
            if i % 25 == 0:
                ...
                print(f"[api] {i}/{len(api_queue)} | ok={counters['ok']} "
                      f"fail={counters['fail']} | {elapsed_min:.1f}m | "
    ...
    if workers == 1:
        for item in api_queue:
            score_one(item)
            time.sleep(SLEEP_BETWEEN)   # serial: keep the polite v9 spacing
    else:
        # parallel: no per-call sleep; concurrency itself paces the API. The
        # in-call retry/backoff in call_sonar handles any 429s.
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futures = [ex.submit(score_one, item) for item in api_queue]
            for _ in as_completed(futures):
                pass

    n_ok, n_fail = counters["ok"], counters["fail"]
