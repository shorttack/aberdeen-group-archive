#!/usr/bin/env python3
"""
union_article_corpus_v2.py

v2 (2026-06-19) — Pipeline-1 routing override
----------------------------------------------
v1 deterministically partitioned every CSV row by headline-match against
the article corpus. The format-mismatch review (2026-06-19) identified
10 row_ids whose quotations landed in real PDF segments but whose
headlines didn't normalize-match (F1/F3/F5/F6 buckets). Pete admitted
them; we need them in Pipeline 1.

v2 reads `_format_mismatch_admits_v1.json` at startup. Any row_id in
the admit list is force-routed to Pipeline 1 regardless of headline
match. If the file is absent, v2 behaves identically to v1.
...
ADMITS_MANIFEST = QUOTATIONS_DIR / "_format_mismatch_admits_v1.json"
...
def main(commit: bool = False):
    ...
    # v2: load format-mismatch admits manifest (Pipeline-1 routing override)
    admit_row_ids = set()
    if ADMITS_MANIFEST.exists():
        try:
            with open(ADMITS_MANIFEST) as f:
                _admits = json.load(f)
            admit_row_ids = {int(rid) for rid in _admits.get("admit_row_ids", [])}
            print(f"  format-mismatch admits manifest: {ADMITS_MANIFEST.name} "
                  f"({len(admit_row_ids)} row_ids force-routed to Pipeline 1)")
        except Exception as e:
            print(f"  WARN: failed to load {ADMITS_MANIFEST.name}: {e}")
            admit_row_ids = set()
    else:
        print(f"  format-mismatch admits manifest: (absent — no overrides)")

    admit_override_count = 0
    for r in rows:
        # v2: check admit override BEFORE headline-match logic
        try:
            rid_int = int(r.get("row_id", ""))
        except (ValueError, TypeError):
            rid_int = None
        if rid_int is not None and rid_int in admit_row_ids:
            pipeline1_rows += 1
            admit_override_count += 1
            continue
    ...
    print(f"  Pipeline 1 rows: {pipeline1_rows} "
          f"(of which {admit_override_count} via format-mismatch admit override)")
    ...
    # v2: sanity — every admitted row_id must be present in rows AND not also
    # in quote_only_rows (i.e., the override must have fired for each)
    if admit_row_ids:
        rows_by_id = {int(r["row_id"]): r for r in rows if r.get("row_id")}
        missing_admits = admit_row_ids - set(rows_by_id.keys())
        if missing_admits:
            sys.exit(f"FATAL: admit manifest references row_ids not in master: {sorted(missing_admits)}")
        leaked_to_p2 = {int(r["row_id"]) for r in quote_only_rows if r.get("row_id")} & admit_row_ids
        if leaked_to_p2:
            sys.exit(f"FATAL: admitted row_ids leaked to Pipeline 2: {sorted(leaked_to_p2)}")
        if admit_override_count != len(admit_row_ids):
            sys.exit(f"FATAL: admit override fired {admit_override_count} times but manifest has {len(admit_row_ids)} row_ids")
